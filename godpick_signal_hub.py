...
SIGNAL_HUB_LOAD_COLUMNS = [
    "股票代號", "股票名稱", "類別", "產業", "推薦日期", "推薦時間", "追蹤更新時間", "最新更新時間",
    "正式推薦分區", "最終操作結論", "操作許可", "正式推薦等級",
    "推薦角色", "飆股雷達角色", "領漲回補角色", "回放校正角色",
    "主流作戰分區", "下週作戰分區", "飆股雷達分區", "領漲回補分區", "回放校正分區",
    "推薦總分", "候選強度分", "股神實戰總分", "可操作分", "實戰操作品質分",
    "爆發雷達分", "隔日爆發分", "飆股攻擊分", "主流領漲回補分", "漲停回放分",
    "強勢股漏選風險分", "族群攻擊強度", "族群輪動分", "主流資金分", "成交額百萬",
    "盤中轉強觸發價", "實戰觸發價", "觸發後守價", "正式推薦排除原因",
]
...
def load_latest_godpick_frame(base_dir: str | Path = ".", *, max_rows: int = 500) -> pd.DataFrame:
    """Read compact recommendation signals without materialising 300+ columns.

    The macro page only needs a small signal summary.  Building a full wide
    DataFrame from records/history JSON caused pandas/PyArrow native crashes and
    unnecessary memory use.  Rows are therefore projected to the required
    columns before DataFrame construction.
    """
    base = Path(base_dir)
    compact_rows: list[dict[str, Any]] = []
    hard_cap = max(100, min(int(max_rows or 500) * 4, 4000))
    for name in SOURCE_FILES:
        ...
        try:
            ...
            for row in rows[:hard_cap]:
                compact = {col: row.get(col) for col in SIGNAL_HUB_LOAD_COLUMNS if col in row}
                compact["資料來源檔"] = name
                compact_rows.append(compact)
                if len(compact_rows) >= hard_cap:
                    break
        except Exception:
            continue
        if len(compact_rows) >= hard_cap:
    ...
    if not compact_rows:
        return pd.DataFrame()
    try:
        out = pd.DataFrame.from_records(compact_rows, columns=list(dict.fromkeys(SIGNAL_HUB_LOAD_COLUMNS + ["資料來源檔"])))
    except Exception:
        # Last-resort object construction: never let a dashboard helper crash the app.
        out = pd.DataFrame({col: pd.Series([row.get(col) for row in compact_rows], dtype="object") for col in list(dict.fromkeys(SIGNAL_HUB_LOAD_COLUMNS + ["資料來源檔"]))})
    if "股票代號" in out.columns:
        out["股票代號"] = out["股票代號"].fillna("").astype("object").map(lambda x: str(x).strip().replace(".0", ""))
        out = out[out["股票代號"].ne("")]
