...
def _phase80_build_recommendation_summary(
    ...
    if formal_n > 0:
        ...
        conclusion = "本輪沒有符合買點、風控與風險報酬門檻的股票；系統選擇空手等待，不用排除股硬湊推薦。"
        status = "空手等待"
    return pd.DataFrame([{
        "本輪結論": status,
        "正式推薦檔數": formal_n,
        ...
        "操作說明": conclusion,
        "核心紀律": "候選總分不等於買進許可；正式推薦必須同時通過 Entry、Risk、RR、追價風險、流動性與大盤風控。",
        "版本": "phase8_0_actionable_recommendation_governance_20260712",
    }])
...
def _phase80_render_actionable_panel(rec_df: pd.DataFrame) -> None:
    ...
    if isinstance(battle, pd.DataFrame) and not battle.empty:
        show_cols = [c for c in [
            ...
            "正式推薦等級", "推薦可信度分", "建議倉位上限%", "可操作分", "Entry進場買點分",
            "Risk風控安全分", "風險報酬比", "實戰觸發價", "觸發後守價", "正式推薦動作",
...
def _build_excel_bytes(
    ...
    formal_summary_export = _phase80_build_recommendation_summary(formal_export, a_minus_export, intraday_core_export, formal_risk_export, formal_exclude_export, len(rec_export) if isinstance(rec_export, pd.DataFrame) else 0)
    ...
    _write_df_to_ws(wb, "股神作戰總表", battle_dashboard_export, "本輪沒有正式推薦、A-準主推薦或盤中核心雷達；系統選擇空手等待。")
    ...
    _write_df_to_ws(wb, "完整推薦表", battle_dashboard_export, "本輪沒有可操作推薦；正式排除股不會寫入完整推薦表。")
    ...
    diag = pd.DataFrame([
        ...
        {"分頁": "完整推薦表", "列數": 0 if battle_dashboard_export is None else len(battle_dashboard_export), "欄數": 0 if battle_dashboard_export is None else len(battle_dashboard_export.columns)},
