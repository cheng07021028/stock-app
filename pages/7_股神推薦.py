...
H45_MAINSTREAM_EXPECTED_VERSION = "v191_h50_1_mainstream_rotation_lifecycle_execution_rr_20260825"
H45_MAINSTREAM_IMPORT_ERROR = ""
try:
    from godpick_mainstream_wave_engine import (
        VERSION as H45_MAINSTREAM_VERSION,
        apply_mainstream_wave_engine,
        build_mainstream_wave_table,
        build_mainstream_sector_lifecycle_table,
    )
except Exception as _h45_main_import_exc:
    H45_MAINSTREAM_VERSION = "h45_mainstream_unavailable"
    H45_MAINSTREAM_IMPORT_ERROR = f"{type(_h45_main_import_exc).__name__}: {_h45_main_import_exc}"
    apply_mainstream_wave_engine = None
    build_mainstream_wave_table = None
    build_mainstream_sector_lifecycle_table = None

H42_DUAL_ROUTE_EXPECTED_VERSION = "v191_h50_1_mainstream_buyable_execution_rr_20260825"
...
H51_HUMAN_MASTER_EXPECTED_VERSION = "v191_h60_mainrise_holder_snowball_truth_20260904"
...
try:
    from godpick_human_master_engine import (
        ...
        build_h62_single_decision_truth_table,
...
except Exception as _h51_import_exc:
    H51_HUMAN_MASTER_VERSION = "h51_human_master_unavailable"
    ...
    build_h62_single_decision_truth_table = None
    ...
    build_h51_mainstream_leader_table = None
    build_h51_sector_table = None

try:
    from godpick_tdcc_holder_service import (
        VERSION as H60_TDCC_VERSION,
        enrich_tdcc_holder_truth,
    )
except Exception:
...
H62_OPPORTUNITY_EXPECTED_VERSION = "v191_h62_incremental_opportunity_effective_formal_truth_20260907"
try:
    from godpick_h62_incremental_opportunity_engine import VERSION as H62_OPPORTUNITY_VERSION, apply_h62_incremental_opportunity_engine
except Exception:
...
H64_CORE_EXPECTED_VERSION = "v191_h64_strong_mainstream_holder_core_truth_20260908"
try:
    from godpick_h64_core_truth_engine import (
        VERSION as H64_CORE_VERSION,
        apply_h64_core_truth,
        build_h64_single_decision_truth_table,
        build_h64_core_research_table,
    )
except Exception:
    H64_CORE_VERSION = "h64_core_unavailable"
    apply_h64_core_truth = None
    build_h64_single_decision_truth_table = None
    build_h64_core_research_table = None

H65_OBSERVATION_EXPECTED_VERSION = "v191_h65_multifactor_observation_radar_20260909"
try:
    from godpick_h65_multifactor_observation_engine import (
        VERSION as H65_OBSERVATION_VERSION,
        apply_h65_multifactor_observation,
        build_h65_observation_radar_table,
        build_h65_indicator_coverage_table,
    )
except Exception:
    H65_OBSERVATION_VERSION = "h65_observation_unavailable"
    apply_h65_multifactor_observation = None
...
H67_GOVERNANCE_EXPECTED_VERSION = "v191_h67_regime_sector_consensus_preopen_truth_20260910"
try:
    from godpick_h67_regime_consensus_engine import (
        VERSION as H67_GOVERNANCE_VERSION,
        apply_h67_regime_consensus,
        build_h67_priority_table,
        build_h67_governance_table,
    )
except Exception:
    H67_GOVERNANCE_VERSION = "h67_governance_unavailable"
    apply_h67_regime_consensus = None
...
H73_LEADERSHIP_EXPECTED_VERSION = "v191_h73_leadership_breadth_distribution_truth_20260916"
try:
    from godpick_h73_leadership_breadth_engine import (
        VERSION as H73_LEADERSHIP_VERSION,
        apply_h73_leadership_breadth,
        build_h73_leadership_table,
        build_h73_governance_summary,
    )
except Exception:
    H73_LEADERSHIP_VERSION = "h73_leadership_unavailable"
    apply_h73_leadership_breadth = None
    build_h73_leadership_table = None
    build_h73_governance_summary = None
...
try:
    from godpick_h74_fresh_mainstream_capital_engine import (
        VERSION as H74_FRESH_CAPITAL_VERSION,
        apply_h74_fresh_mainstream_capital,
        build_h74_research_table,
        build_h74_decision_overview,
        build_h74_excel_guide,
        build_h74_governance_summary,
    )
except Exception:
    H74_FRESH_CAPITAL_VERSION = "h74_fresh_capital_unavailable"
    apply_h74_fresh_mainstream_capital = None
    build_h74_research_table = None
    build_h74_decision_overview = None
    build_h74_excel_guide = None
    build_h74_governance_summary = None

H75_EXECUTIVE_EXPECTED_VERSION = "v191_h75_executive_decision_export_20260917"
try:
    from godpick_h75_executive_decision_export import (
        VERSION as H75_EXECUTIVE_VERSION,
        apply_h75_executive_decision,
        build_h75_today_ai_table,
        build_h75_sector_summary,
        build_h75_evidence_table,
        build_h75_performance_summary,
        build_h75_health_summary,
    )
except Exception:
    H75_EXECUTIVE_VERSION = "h75_executive_unavailable"
    apply_h75_executive_decision = None
    build_h75_today_ai_table = None
    build_h75_sector_summary = None
    build_h75_evidence_table = None
    build_h75_performance_summary = None
    build_h75_health_summary = None

H76_DAILY_ALPHA_EXPECTED_VERSION = "v191_h76_daily_alpha_core_split_repeat_evidence_truth_20260918"
try:
    from godpick_h76_daily_alpha_core_split import (
        VERSION as H76_DAILY_ALPHA_VERSION,
        apply_h76_daily_alpha_core_split,
        build_h76_daily_alpha_table,
        build_h76_structural_core_table,
        build_h76_evidence_table,
        build_h76_performance_health_summary,
        build_h76_governance_summary,
    )
except Exception:
    H76_DAILY_ALPHA_VERSION = "h76_daily_alpha_unavailable"
    apply_h76_daily_alpha_core_split = None
    build_h76_daily_alpha_table = None
    build_h76_structural_core_table = None
    build_h76_evidence_table = None
    build_h76_performance_health_summary = None
    build_h76_governance_summary = None

# H78 is the primary opportunity board; import errors are visible, never hidden.
try:
...
PAGE07_SPEED_FIX_VERSION = "page07_v191_h78_evidence_contract_decision_rebuild_20260920"
...
def _v191_actionable_tracking_frame(source_df: pd.DataFrame | None) -> tuple[pd.DataFrame, bool, list[str]]:
    ...
    # H62 action/persistence truth: raw Formal remains in the diagnostic frame,
    # but FORMAL-HOLD must not be written to Page08/Page10 as this-run Formal.
    # A future run can restore EFFECTIVE-FORMAL automatically.
    try:
        if callable(apply_human_master_engine):
            _h62v = work.get("H62版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h62v.eq(H62_OPPORTUNITY_VERSION).all():
                work = apply_human_master_engine(work)
        elif callable(apply_h62_incremental_opportunity_engine):
            work = apply_h62_incremental_opportunity_engine(work)
    except Exception as _h62_action_exc:
        notes.append(f"H62有效Formal治理暫時無法套用：{_h62_action_exc}")
    # H64: an H62 EFFECTIVE-FORMAL still must pass current strength, current
    # mainstream and verified holder-lock truth before entering Page08/Page10.
    try:
        if callable(apply_h64_core_truth):
            _h64v = work.get("H64版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h64v.eq(H64_CORE_VERSION).all():
                work = apply_h64_core_truth(work)
    except Exception as _h64_action_exc:
        notes.append(f"H64核心品質治理暫時無法套用：{_h64_action_exc}")
    ...
    if "H62有效權威" in work.columns:
        _h62_eff_action = work["H62有效權威"].fillna("").astype(str)
        _raw_formal_bucket = bucket.eq("正式下週主推薦")
        _held_formal = _raw_formal_bucket & _h62_eff_action.eq("FORMAL-HOLD")
        if bool(_held_formal.any()):
            notes.append(f"H62有效Formal治理：{int(_held_formal.sum())} 檔原始Formal因近期Alpha/增量機會不足改為FORMAL-HOLD，本輪不寫入正式作戰路徑。")
        formal_mask = formal_mask & ~_held_formal
    if "H64有效權威" in work.columns:
        _h64_eff_action = work["H64有效權威"].fillna("").astype(str)
        _raw_formal_bucket = bucket.eq("正式下週主推薦")
        _h64_quality_hold = _raw_formal_bucket & ~_h64_eff_action.eq("EFFECTIVE-FORMAL")
        if bool(_h64_quality_hold.any()):
            notes.append(f"H64核心品質治理：{int(_h64_quality_hold.sum())} 檔原始Formal未同時通過當前強勢×主流×鎖碼真相，本輪不寫入正式作戰路徑。")
        formal_mask = formal_mask & ~_h64_quality_hold
...
def _operational_recommendation_rows(df: pd.DataFrame | None, *, refresh_decision: bool = False) -> pd.DataFrame:
    ...
    if governed.empty:
        return governed
    try:
        if callable(apply_h62_incremental_opportunity_engine):
            governed = apply_h62_incremental_opportunity_engine(governed)
        if callable(apply_h64_core_truth):
            governed = apply_h64_core_truth(governed)
        if "正式推薦分區" in governed.columns:
            _gbucket = governed["正式推薦分區"].fillna("").astype(str)
            if "H62有效權威" in governed.columns:
                _geff = governed["H62有效權威"].fillna("").astype(str)
                governed = governed.loc[~(_gbucket.eq("正式下週主推薦") & _geff.eq("FORMAL-HOLD"))].copy()
            if "H64有效權威" in governed.columns:
                _g64 = governed["H64有效權威"].fillna("").astype(str)
                governed = governed.loc[~(_gbucket.eq("正式下週主推薦") & ~_g64.eq("EFFECTIVE-FORMAL"))].copy()
    except Exception:
        pass
    if governed.empty:
        return governed
...
def _analyze_stock_bundle(stock_no: str, stock_name: str, market_type: str, start_dt: date, end_dt: date, risk_strictness: str) -> dict[str, Any]:
    try:
        ...
        amount_accel_3 = _pct_accel(positive_amount, 3) if not positive_amount.empty else float("nan")
        amount_accel_5 = _pct_accel(positive_amount, 5) if not positive_amount.empty else float("nan")
        volume_accel_3 = _pct_accel(positive_volume, 3) if not positive_volume.empty else float("nan")
        volume_accel_5 = _pct_accel(positive_volume, 5) if not positive_volume.empty else float("nan")

        ret_series = valid_close.pct_change().replace([np.inf, -np.inf], np.nan).dropna()
        rv5 = (_safe_float(ret_series.tail(5).std(), 0) or 0) * 100.0 if len(ret_series) >= 3 else float("nan")
        rv20 = (_safe_float(ret_series.tail(20).std(), 0) or 0) * 100.0 if len(ret_series) >= 5 else float("nan")
        prior_ret_series = ret_series.iloc[:-1] if len(ret_series) >= 2 else ret_series.iloc[0:0]
        prior_rv5 = (_safe_float(prior_ret_series.tail(5).std(), 0) or 0) * 100.0 if len(prior_ret_series) >= 3 else float("nan")
        prior_rv20 = (_safe_float(prior_ret_series.tail(20).std(), 0) or 0) * 100.0 if len(prior_ret_series) >= 5 else float("nan")
...
def _analyze_one_stock_for_recommend(
    ...
    return {
        ...
        "row": {
            ...
            "成交額3日加速度%": round(_safe_float(bundle.get("amount_accel_3"), float("nan")), 2),
            "成交額5日加速度%": round(_safe_float(bundle.get("amount_accel_5"), float("nan")), 2),
            "成交量3日加速度%": round(_safe_float(bundle.get("volume_accel_3"), float("nan")), 2),
            "成交量5日加速度%": round(_safe_float(bundle.get("volume_accel_5"), float("nan")), 2),
            ...
            "3日動能加速度百分點": round(_safe_float(bundle.get("momentum_accel_3pp"), float("nan")), 3),
...
def _compute_category_strength(base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # H45：類股主流必須看「現在」而不是任意長區間。舊版直接沿用
    # 區間漲跌幅%，會出現PCB 249%、被動元件208%這種歷史累積值，
    # 嚴重污染類股熱度。改用1/5/20日加權近端動能，原欄位保留不改。
    _ret1_sector = _numeric_col("今日漲幅%").fillna(_numeric_col("當日漲幅%")).fillna(0).clip(-12, 12)
    _ret5_sector = _numeric_col("近5日漲幅%").fillna(0).clip(-25, 25)
    _ret20_sector = _numeric_col("近20日漲幅%").fillna(0).clip(-40, 40)
    work["_sector_short_momentum_pct"] = (
        _ret1_sector * 0.20 + _ret5_sector * 0.50 + _ret20_sector * 0.30
    )
    ...
    grp = (
        ...
        .agg(
            ...
            類股平均總分=("個股原始總分", "mean"),
            類股平均訊號=("訊號分數", "mean"),
            類股平均漲幅=("_sector_short_momentum_pct", "mean"),
            類股平均雷達=("雷達均分", "mean"),
    ...
    try:
        ...
        if callable(apply_super_ai_engine):
            try:
                ...
                # H45：V188後先辨識「當前主流族群＋領先股＋波段起漲結構」，
                # 再交給H34判斷是否值得成為每日安全精選。這修正舊版Trade/RR
                # 排得很前、但股票本身不是主流也沒有起漲感的問題。
                if callable(apply_mainstream_wave_engine):
                    governed_candidate_df = apply_mainstream_wave_engine(governed_candidate_df)
                    debug_summary["h45_mainstream_wave_applied"] = True
                else:
                    debug_summary["h45_mainstream_wave_applied"] = False
                    debug_summary["h45_mainstream_wave_error"] = "H45 mainstream engine unavailable"

                # H38/H45：H34 必須在 SuperAI/V188 + H45主流波段完成後才執行。
                if callable(apply_daily_safe_selection):
                    governed_candidate_df = apply_daily_safe_selection(governed_candidate_df)
                    debug_summary["h45_h34_post_mainstream"] = True
                else:
                    debug_summary["h45_h34_post_mainstream"] = False
                    debug_summary["h45_h34_error"] = "H34 post-H45 engine unavailable"

                # V191-H42/H45：在 Formal/V188/H45/H34 權威決策完成後，再建立「強勢延續 /
                ...
                # H60：正式H51前先嘗試補 TDCC 千張大戶真相。網路失敗不阻斷推薦，
                # H60會明確回退 PROXY，絕不把代理籌碼冒充官方大戶持股。
                if callable(enrich_tdcc_holder_truth):
                    try:
                        governed_candidate_df = enrich_tdcc_holder_truth(governed_candidate_df, allow_network=True, timeout=3.5)
                        debug_summary["h60_tdcc_holder_status"] = "ACTUAL_OR_CACHE" if "TDCC大戶資料狀態" in governed_candidate_df.columns else "NO_COLUMN"
                        debug_summary["h60_tdcc_actual_rows"] = int(governed_candidate_df.get("TDCC大戶資料狀態", pd.Series([], dtype=str)).astype(str).eq("ACTUAL").sum()) if "TDCC大戶資料狀態" in governed_candidate_df.columns else 0
                    except Exception as _h60_tdcc_exc:
                        debug_summary["h60_tdcc_holder_status"] = f"FALLBACK_PROXY｜{type(_h60_tdcc_exc).__name__}: {_h60_tdcc_exc}"

                # H51~H60：真人高手式決策/研究層。市場/族群 -> 真領漲 -> Pivot/回檔再攻 ->
                # H57前兆 -> H60主升/大戶鎖碼/雪球。只新增研究與交易許可說明，不覆寫Formal/V188/H34。
                if callable(apply_human_master_engine):
                    governed_candidate_df = apply_human_master_engine(governed_candidate_df)
                    debug_summary["h60_human_master_applied"] = True
                else:
                    debug_summary["h60_human_master_applied"] = False
                    debug_summary["h60_human_master_error"] = H51_HUMAN_MASTER_IMPORT_ERROR or "H60/H51 engine unavailable"
        ...
        # H78: once per completed scan, retain true decision-time evidence.
        try:
            from godpick_h78_snapshot_store import capture_snapshot
            debug_summary["h78_research_snapshot"] = capture_snapshot(governed_candidate_df)
        except Exception as _h78_save_error:
            debug_summary["h78_research_snapshot_error"] = str(_h78_save_error)
            st.warning(f"H78學習快照保存失敗：{_h78_save_error}")
...
def _build_export_views(rec_df: pd.DataFrame, category_strength_df: pd.DataFrame, top_n: int, full_order: list[str] | None = None):
    ...
    # H50：類股強度榜以「目前主流生命週期 / 可買新鮮度」為權威。
    # 舊H45持續強勢分會讓成熟族群（例如航運）因過去量能/攻擊分長期霸榜；
    # H50改用可買主流分、新鮮度、回檔再攻、起漲候選比例與重複壓力排序。
    if "類別" in rec_df.columns and "H50族群可買主流分" in rec_df.columns:
        try:
            _h50_sector_cols = [c for c in [
                "H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分",
                "H50族群起漲候選比例%", "H50族群延伸過熱比例%", "H50族群重複壓力分",
                "H50族群生命週期", "H45族群主流分", "H45族群主流排名", "H45族群狀態",
                "H45族群短線動能%", "H45族群5日上漲比例%", "H45族群20日上漲比例%",
                "H45族群量能分", "H45族群領先比例%", "H45族群樣本數",
            ] if c in rec_df.columns]
            _sector_src = rec_df[["類別"] + _h50_sector_cols].copy()
            ...
            cat_export = _safe_sort_export_df(
                cat_export,
                ["H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分", "H50族群起漲候選比例%", "H45族群主流分"],
                [False, False, False, False, False],
            ).head(top_n).copy()
            cat_export.insert(1, "H50主流閱讀方式", "先看生命週期與可買新鮮度；C成熟/高檔輪動不因過去強勢長期霸榜，B1回檔蓄勢/A0新主流點火優先")
        except Exception:
            cat_export = pd.DataFrame()
    elif "類別" in rec_df.columns and "H45族群主流分" in rec_df.columns:
        try:
            _legacy_sector_cols = [c for c in [
                "H45族群主流分", "H45族群主流排名", "H45族群主流百分位%", "H45族群狀態",
                "H45族群短線動能%", "H45族群5日上漲比例%", "H45族群20日上漲比例%",
                "H45族群量能分", "H45族群領先比例%", "H45族群樣本數",
            ] if c in rec_df.columns]
            _sector_src = rec_df[["類別"] + _legacy_sector_cols].copy()
            _sector_src["類別"] = _sector_src["類別"].fillna("未分類").astype(str)
            _sector_src = _sector_src.loc[_sector_src["類別"].str.strip().ne("")]
            cat_export = _sector_src.drop_duplicates(subset=["類別"], keep="first").copy()
            cat_export = _safe_sort_export_df(cat_export, ["H45族群主流分", "H45族群主流百分位%", "H45族群量能分"], [False, False, False]).head(top_n).copy()
            cat_export.insert(1, "H47主流閱讀方式", "舊資料缺H50時才退回H45；請重新推薦取得H50生命週期判斷")
        except Exception:
            cat_export = pd.DataFrame()
    ...
    if isinstance(cat_export, pd.DataFrame) and not cat_export.empty:
        if "H50族群可買主流分" in cat_export.columns:
            cat_export = _safe_sort_export_df(cat_export, ["H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分"], [False, False, False]).head(top_n).copy()
        elif "H45族群主流分" not in cat_export.columns:
        ...
        # v72：即使主流引擎資料缺失，也直接從推薦表用最基本欄位重建類股榜，避免分頁空白。
        try:
            base = rec_df.copy()
            ...
            if cat_col:
                if score_col:
                    ...
                    cat_export = (
                        ...
                        .reset_index().rename(columns={cat_col: "類別"})
                        .sort_values(["類股平均總分", "股票數"], ascending=[False, False]).head(top_n)
                    )
                else:
                    cat_export = (
                        base.groupby(cat_col, dropna=False).size().reset_index(name="股票數")
                        .rename(columns={cat_col: "類別"}).sort_values("股票數", ascending=False).head(top_n)
                    )
        except Exception:
            cat_export = pd.DataFrame()

    # H50：同類股領先榜先看「新鮮主流 + 起漲/回檔階段 + 可買性」，
    # 再看H47純市場地位。成熟主流與已延伸領漲股仍保留研究價值，但不得長期霸榜。
    if "H50主流購買狀態" in rec_df.columns:
        leader_df = rec_df.copy()
        _h50s = leader_df["H50主流購買狀態"].fillna("").astype(str)
        _h50stage = leader_df.get("H50波段機會階段", pd.Series([""] * len(leader_df), index=leader_df.index)).fillna("").astype(str)
        leader_df["_H50新鮮主流優先"] = _h50s.map(
            lambda x: 7 if x.startswith("F-SETUP") else 6 if x.startswith("F-LEADER") else 4 if x.startswith("F-RADAR") else 2 if x.startswith("F-MATURE") else 1 if x.startswith("F-NO-CHASE") else 0
        )
        leader_df["_H50階段優先"] = _h50stage.map(
            lambda x: 7 if x.startswith("N-EARLY") else 6 if x.startswith("N-PULLBACK") else 5 if x.startswith("N-LEADER") else 2 if x.startswith("N-MATURE") else 1 if x.startswith("N-EXTENDED") else 0
        )
        leader_df = _safe_sort_export_df(
            leader_df,
            ["_H50新鮮主流優先", "_H50階段優先", "H50主流購買優先分", "H50族群可買主流分", "H47個股相對強度分", "H47起漲優先分"],
            [False, False, False, False, False, False],
        )
    elif "H47主流領先狀態" in rec_df.columns:
        leader_df = rec_df.copy()
    ...
    leader_cols = [
        "股票代號", "股票名稱", "市場別", "類別",
        "H50主流購買狀態", "H50波段機會階段", "H50主流購買優先分", "H50主流購買理由",
        "H50族群生命週期", "H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分", "H50重複推薦扣分",
        "H47主流領先狀態", "H47交易候選狀態", "H47起漲優先分", "H47個股相對強度分",
        "H47族群內領先排名", "H47族群內領先百分位%", "H47主流領先理由",
        "H45族群主流分", "H45族群主流排名", "H45族群主流百分位%", "H45族群狀態",
...
def _write_df_to_ws(wb, sheet_name: str, df: pd.DataFrame, fallback_title: str):
    ...
    try:
        ...
        if safe_name in {"01_今日推薦與計畫", "02_等待與核心監控", "03_今日主流資金", "04_驗證與風險證據", "05_績效煞車與健康", "AI決策總覽", "Excel閱讀指南", "H74新鮮主流資金", "H74治理摘要", "超級AI最終決策", "正式推薦作戰", "H73領先共振", "H73治理摘要", "H72多模型Alpha", "H72治理摘要", "H70逆勢Alpha觀察", "H70治理摘要", "H67次日優先治理", "H67治理摘要", "T+1時機雷達", "H66學習治理", "多因子觀察雷達", "股神推薦總排名"}:
            ...
            _rank_headers = {"01_今日推薦與計畫":"今日驗證Alpha順位","02_等待與核心監控":"等待監控順位","04_驗證與風險證據":"證據順位","AI決策總覽":"AI決策順位","H74新鮮主流資金":"H74全市場順位","超級AI最終決策":"決策順位","正式推薦作戰":"H63正式推薦順位","H67次日優先治理":"H67研究順位","T+1時機雷達":"H66T1觀察順位","多因子觀察雷達":"H65觀察順位"}
...
def _build_record_rows_from_rec_df(rec_df: pd.DataFrame, selected_codes: list[str]) -> list[dict[str, Any]]:
    ...
    if callable(apply_h72_multi_model_alpha_ensemble):
        try:
            ...
            pass
    if callable(apply_h73_leadership_breadth):
        try:
            _record_source = apply_h73_leadership_breadth(_record_source)
        except Exception:
            pass
...
def _phase90_build_master_recommendation_rank(source_df: pd.DataFrame, top_n: int = 30) -> pd.DataFrame:
    ...
    if work.empty or "股票代號" not in work.columns:
        return pd.DataFrame()
    # H45：研究總排名先回答「是不是當前主流、是不是波段起漲」，
    # 再看V188 Trade/RR決定能不能做。這不改Formal權限，只改研究優先序。
    if callable(apply_mainstream_wave_engine):
        try:
            _h45_ver = work.get("H45版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h45_ver.eq(H45_MAINSTREAM_VERSION).all():
    ...
    # H50：研究總排名同時保留「真正主流」與「目前可買性」，但不再讓
    # 成熟高量族群因歷史強度反覆霸榜。V188<50的 fresh-mainstream 雷達可保留
    # 研究，交易許可仍獨立標示。
    if callable(apply_dual_opportunity_engine):
        try:
            _h50v = work.get("H50版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h50v.eq(H42_DUAL_ROUTE_VERSION).all():
    ...
    if callable(apply_human_master_engine):
        try:
            ...
            _h62v = work.get("H62版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            _h64v = work.get("H64版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if (not _h51v.eq(H51_HUMAN_MASTER_VERSION).all()) or (not _h61v.eq(H61_OPPORTUNITY_VERSION).all()) or (not _h62v.eq(H62_OPPORTUNITY_VERSION).all()) or (not _h64v.eq(H64_CORE_VERSION).all()):
                work = apply_human_master_engine(work)
        except Exception:
            pass
    if callable(apply_h65_multifactor_observation):
        try:
            _h65v = work.get("H65版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h65v.eq(H65_OBSERVATION_VERSION).all():
    ...
    if callable(apply_h66_adaptive_timing):
        try:
            ...
            pass
    if callable(apply_h67_regime_consensus):
        try:
            _h67v = work.get("H67版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h67v.eq(H67_GOVERNANCE_VERSION).all():
    ...
    if callable(apply_h73_leadership_breadth):
        try:
            _h73v = work.get("H73版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h73v.eq(H73_LEADERSHIP_VERSION).all():
                work = apply_h73_leadership_breadth(work)
        except Exception:
            pass
    if callable(apply_h74_fresh_mainstream_capital):
        try:
            _h74v = work.get("H74版本", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str)
            if not _h74v.eq(H74_FRESH_CAPITAL_VERSION).all():
                work = apply_h74_fresh_mainstream_capital(work)
        except Exception:
            pass
    ...
    _fresh_keep = _h50_research_status.str.startswith(("F-SETUP", "F-LEADER", "F-RADAR")) | _h50_recommend.str.startswith(("R-READY", "R-PREP", "R-RADAR"))
    ...
    _h73_tier_keep = work.get("H73研究層級", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str).str.startswith(("L1", "L2", "L3"))
    _h74_tier_keep = work.get("H74研究層級", pd.Series([""] * len(work), index=work.index)).fillna("").astype(str).str.startswith(("F1", "F2", "F3"))
    keep &= (score.ge(50.0) | _fresh_keep | _h47_research_keep | _h51_keep | _h55_fresh_keep | _h57_fresh_keep | _h65_tier_keep | _h66_tier_keep | _h67_tier_keep | _h72_tier_keep | _h73_tier_keep | _h74_tier_keep)
    keep &= ~freshness.str.contains("過期|落後|待更新", regex=True, na=False)
    rank = work.loc[keep].copy()
    if rank.empty:
        return pd.DataFrame()

    def _h50_recommend_priority(text: str) -> int:
        if text.startswith("R-READY"): return 9
        if text.startswith("R-PREP"): return 8
        if text.startswith("R-RADAR"): return 6
        if text.startswith("R-NO-CHASE"): return 3
        if text.startswith("R-MATURE"): return 1
        return 0
    def _h50_fresh_priority(text: str) -> int:
        if text.startswith("F-SETUP"): return 7
        if text.startswith("F-LEADER"): return 6
        if text.startswith("F-RADAR"): return 5
        if text.startswith("F-NO-CHASE"): return 2
        if text.startswith("F-MATURE"): return 1
        return 0
    rank["_H50推薦優先"] = rank.get("H50推薦決策", pd.Series([""] * len(rank), index=rank.index)).fillna("").astype(str).map(_h50_recommend_priority)
    ...
    # H74: fresh leadership/capital rotation is the first non-Formal research rank.
    # It explicitly prevents stable historic quality from dominating when today's mainline/flow has rotated.
    _h74_tier_rank = rank.get("H74研究層級", pd.Series([""] * len(rank), index=rank.index)).fillna("").astype(str)
    rank["_H74研究優先"] = _h74_tier_rank.map(lambda x: 80 if x.startswith("F1") else 62 if x.startswith("F2") else 38 if x.startswith("F3") else 10 if x.startswith("R0") else 0)
    # H73 remains supporting leadership/breadth evidence under H74.
    _h73_tier_rank = rank.get("H73研究層級", pd.Series([""] * len(rank), index=rank.index)).fillna("").astype(str)
    rank["_H73研究優先"] = _h73_tier_rank.map(lambda x: 70 if x.startswith("L1") else 55 if x.startswith("L2") else 35 if x.startswith("L3") else 10 if x.startswith("R0") else 0)
    # H72 remains the structural multi-model base under H73.
    ...
    rank["_H67研究優先"] = _h67_tier_rank.map(lambda x: 50 if x.startswith("P1") else 40 if x.startswith("P2") else 25 if x.startswith("C1") else 10 if x.startswith("R0") else 0)
    ...
    rank["_H64前排優先"] = (~_h64_front.str.startswith("否") & ~_h64_eff.isin(["FORMAL-HOLD", "FORMAL-QUALITY-HOLD"])).astype(int)
    # H62: every stock competes for scarce attention against today's market.
    # Raw Formal on FORMAL-HOLD remains auditable but is not allowed to dominate
    # the research ranking merely because it was Formal yesterday.
    _h62_front = rank.get("H62前排資格", pd.Series(["否"] * len(rank), index=rank.index)).fillna("否").astype(str)
    _h62_eff = rank.get("H62有效權威", pd.Series([""] * len(rank), index=rank.index)).fillna("").astype(str)
    rank["_H62有效Formal優先"] = _h62_eff.eq("EFFECTIVE-FORMAL").astype(int)
    rank["_H62前排優先"] = (~_h62_front.str.startswith("否") & ~_h62_eff.eq("FORMAL-HOLD")).astype(int)
    # H61 remains supporting evidence under H62.
    ...
    sort_cols = [
        "_H64有效Formal優先", "_H74研究優先", "H74決策總分", "H74全市場百分位%", "H74法人資金加速度分", "H74成交資金加速度分", "H74主流新鮮度分", "H74強勢加速度分",
        "_H73研究優先", "H73研究排序分", "H73全市場百分位%", "H73領先加速分", "H73族群廣度分", "H73模型一致性分",
        "_H72研究優先", "H72風險調整分", "H72全市場百分位%", "H72共振模型數", "H72模型覆蓋%",
        ...
        "進場可執行分", "強勢動能分", "強勢前兆分", "主流資金分", "族群攻擊強度", "流動性參考成交額百萬",
    ]
    ...
    # H50 sector diversification: top10 normally max2/sector, top20 max3/sector.
    # If the market genuinely has too few sectors, relax one step only to fill.
    _target = max(1, int(top_n or 30))
    _rows = []
    _chosen = set()
    _counts = {}
    _records = list(rank.iterrows())
    def _add_with_cap(target_n: int, cap: int):
        for _idx, _row in _records:
            if len(_rows) >= target_n:
                break
            _code = _safe_str(_row.get("股票代號"))
            _sector = _safe_str(_row.get("類別")) or _safe_str(_row.get("族群名稱")) or "未分類"
            if not _code or _code in _chosen or _counts.get(_sector, 0) >= cap:
                continue
            _rows.append(_row); _chosen.add(_code); _counts[_sector] = _counts.get(_sector, 0) + 1
    _add_with_cap(min(_target, 10), 2)
    if len(_rows) < min(_target, 10):
        _add_with_cap(min(_target, 10), 3)
    _add_with_cap(min(_target, 20), 3)
    if len(_rows) < min(_target, 20):
        _add_with_cap(min(_target, 20), 4)
    _add_with_cap(_target, 4)
    ...
    rank.drop(columns=["_H64有效Formal優先", "_H73研究優先", "_H72研究優先", "_H67研究優先", "_H66T1優先", "_H65觀察優先", "_H64前排優先", "_H62有效Formal優先", "_H62前排優先", "_H61前排優先", "_H57總覽優先", "_H55總覽優先", "_H51交易優先", "_H51市場優先", "_H50推薦優先", "_H50新鮮主流優先", "_H47市場地位優先"], inplace=True, errors="ignore")
...
def _phase90_navigation_table() -> pd.DataFrame:
    return pd.DataFrame([
        ...
        {"優先序": 2, "活頁/表格": "H73領先共振", "真正用途": "H72之上的橫截面領先排序：族群廣度、加速、Regime轉折與分布矛盾", "是否買進清單": "否｜L1/L2/L3均為研究層，不建立Formal"},
        {"優先序": 3, "活頁/表格": "H72多模型Alpha", "真正用途": "八模型共識＋Regime動態權重：成長、品質、法人、大戶、估值、動能、突破、風險", "是否買進清單": "否｜E1/E2/E3均為研究層，不建立Formal"},
...
def _phase93_single_source_decision_frame(
    ...
    if (
        ...
        # V191-H45：快取若沒有當前主流波段版本，代表仍是Trade/RR-first的舊決策，
        # 必須重建，否則元太/義隆類單日反彈仍可能被稱為強勢。
        if _cache_ok:
            _h45_mark = _cached_decision.get(
                "H45版本",
                pd.Series([""] * len(_cached_decision), index=_cached_decision.index),
            ).fillna("").astype(str)
            _cache_ok = bool(_h45_mark.eq(H45_MAINSTREAM_VERSION).all())
            if not _cache_ok:
                _cache_diag = {"complete": False, "reason": "V191-H50主流生命週期/新鮮度尚未重建"}

        # V191-H42/H45：舊 decision frame 可能保留 H41 時期的單一低分紅燈/LOCKDOWN。
        ...
        if _cache_ok:
            ...
            if not _cache_ok:
                _cache_diag = {"complete": False, "reason": "V191-H50主流可買推薦/雙路徑尚未重建"}

        if _cache_ok:
            _h51_mark = _cached_decision.get(
                "H51版本",
                pd.Series([""] * len(_cached_decision), index=_cached_decision.index),
            ).fillna("").astype(str)
            _cache_ok = bool(_h51_mark.eq(H51_HUMAN_MASTER_VERSION).all())
            if not _cache_ok:
                _cache_diag = {"complete": False, "reason": "V191-H51真人主線/Pivot/執行分離尚未重建"}
        ...
        if not _cache_ok and callable(repair_v188_decision_frame) and callable(apply_super_ai_engine):
            try:
                ...
                if callable(apply_mainstream_wave_engine):
                    _repaired = apply_mainstream_wave_engine(_repaired)
                if callable(apply_dual_opportunity_engine):
                    _repaired = apply_dual_opportunity_engine(_repaired)
                if callable(apply_human_master_engine):
                    _repaired = apply_human_master_engine(_repaired)
    ...
    if callable(repair_v188_decision_frame) and callable(apply_super_ai_engine):
        try:
            ...
            if callable(apply_mainstream_wave_engine):
                source = apply_mainstream_wave_engine(source)
            if callable(apply_dual_opportunity_engine):
                source = apply_dual_opportunity_engine(source)
            if callable(apply_human_master_engine):
                source = apply_human_master_engine(source)
...
def _phase80_render_actionable_panel(rec_df: pd.DataFrame) -> None:
    ...
    render_pro_section("超級AI股神｜H78 推薦與交易計畫")
    st.caption("依當期相對強度、族群共振、廣度、量能與法人方向選出值得研究的股票；推薦研究與可執行交易分開計數。固定初始政策尚未經外樣本驗證。")
    try:
        if not callable(build_h78_tables):
            raise RuntimeError(H78_IMPORT_ERROR or "H78引擎未部署")
        _h78_tables = build_h78_tables(decision_source)
        st.dataframe(_format_df(_h78_tables["recommendations"]), use_container_width=True, hide_index=True)
        st.dataframe(_format_df(_h78_tables["health"]), use_container_width=True, hide_index=True)
        with st.expander("完整候選淘汰原因與價格計畫", expanded=False):
            st.dataframe(_format_df(_h78_tables["audit"]), use_container_width=True, hide_index=True)
        st.caption("升級後請重新推薦一次，補回舊診斷快照已遺失的原始增量欄位。舊Excel無法還原不存在的K線增量。")
    except Exception as _h78_error:
        st.error(f"H78決策建立失敗（不是沒有機會）：{type(_h78_error).__name__}: {_h78_error}")
    with st.expander("舊版決策與交易授權診斷", expanded=False):
        # H64：真正第一屏只回答「當前強勢、當前主流、鎖碼趨勢是否同時成立」。
        # 高分、熟面孔、單點TDCC持股都不能再冒充核心候選。
        render_pro_section("超級AI唯一決策｜H64 強勢×主流×大戶鎖碼真相")
        st.caption("H64把H42/H47當前強勢、H51/H53/H57主流形成、TDCC前後期千張大戶變化與RR放到同一硬閘門。高持股但沒有前期差異只能叫『持股已知』，不是鎖碼；非強勢/非主流即使H62增量高也不能霸榜。")
        _h64_engine_ok = bool(
            callable(apply_human_master_engine)
            and callable(build_h64_single_decision_truth_table)
            and callable(apply_h64_core_truth)
            ...
            and H64_CORE_VERSION == H64_CORE_EXPECTED_VERSION
        )
        try:
            _h51_source_ui = apply_human_master_engine(decision_source) if _h64_engine_ok else decision_source
            if _h64_engine_ok:
                _h51_source_ui = apply_h64_core_truth(_h51_source_ui)
            _h64_focus = build_h64_single_decision_truth_table(_h51_source_ui, max_rows=10) if _h64_engine_ok else pd.DataFrame({"H64唯一決策": ["H64未完整部署｜不是沒有推薦。"]})
        except Exception as _h64_ui_exc:
            _h51_source_ui = decision_source
            _h64_focus = pd.DataFrame({"H64唯一決策": [f"H64唯一決策暫時無法建立：{_h64_ui_exc}"]})
        if isinstance(_h64_focus, pd.DataFrame) and not _h64_focus.empty:
            _h64_tier_ui = _h64_focus.get("H64唯一決策", pd.Series([""] * len(_h64_focus))).fillna("").astype(str)
            render_pro_kpi_row([
                {"label": "F1核心正式", "value": str(int(_h64_tier_ui.str.startswith("F1").sum())), "delta": "強勢×主流×鎖碼＋Formal"},
                {"label": "C2核心研究", "value": str(int(_h64_tier_ui.str.startswith("C2").sum())), "delta": "三真相對齊但未正式授權"},
                {"label": "C3待鎖碼", "value": str(int(_h64_tier_ui.str.startswith("C3").sum())), "delta": "強勢主流成立，等TDCC趨勢"},
                {"label": "E1新主流", "value": str(int(_h64_tier_ui.str.startswith("E1").sum())), "delta": "形成中研究，不可直接買"},
            ])
            st.dataframe(_format_df(_h64_focus), use_container_width=True, hide_index=True)
            _h64_eff_all = _h51_source_ui.get("H64有效權威", pd.Series([""] * len(_h51_source_ui), index=_h51_source_ui.index)).fillna("").astype(str)
            _h64_raw_formal_ui = int(_h51_source_ui.get("H62原始權威", _h51_source_ui.get("H56上游權威層級", pd.Series([""] * len(_h51_source_ui), index=_h51_source_ui.index))).fillna("").astype(str).eq("FORMAL").sum())
            _h64_effective_formal_ui = int(_h64_eff_all.eq("EFFECTIVE-FORMAL").sum())
        ...
        render_pro_section("超級AI次日治理｜H67 Regime × Sector × Consensus")
        st.caption("H67不增加Formal。它專門修正H66在弱市場/族群退潮時A1過寬的問題，並把追價耗竭與次日盤前重驗變成明確治理。")
        _h67_engine_ok = bool(
            callable(apply_h67_regime_consensus) and callable(build_h67_priority_table)
            and callable(build_h67_governance_table)
            and H67_GOVERNANCE_VERSION == H67_GOVERNANCE_EXPECTED_VERSION
        )
        try:
            if _h67_engine_ok:
                _h51_source_ui = apply_h67_regime_consensus(_h51_source_ui)
                _h67_ui = build_h67_priority_table(_h51_source_ui, max_rows=20, max_per_sector=2)
            else:
                _h67_ui = pd.DataFrame({"狀態": [f"H67治理引擎未完整部署：{H67_GOVERNANCE_VERSION}/{H67_GOVERNANCE_EXPECTED_VERSION}"]})
        except Exception as _h67_ui_exc:
            _h67_ui = pd.DataFrame({"狀態": [f"H67治理雷達建立失敗：{type(_h67_ui_exc).__name__}: {_h67_ui_exc}"]})
        if isinstance(_h67_ui, pd.DataFrame) and not _h67_ui.empty:
            _h67tier_ui = _h67_ui.get("H67研究優先層級", pd.Series([""] * len(_h67_ui))).fillna("").astype(str)
            ...
            render_pro_kpi_row([
                ...
                {"label": "族群資金調整", "value": (f"{float(_h67sector_ui.median()):+.1f}" if not _h67sector_ui.dropna().empty else "-"), "delta": "退潮會降級"},
            ])
            st.dataframe(_format_df(_h67_ui), use_container_width=True, hide_index=True)
        with st.expander("H67治理規則｜為何A1不再大量膨脹", expanded=False):
            try:
                _h67_gov_ui = build_h67_governance_table(_h51_source_ui) if _h67_engine_ok else pd.DataFrame()
                if isinstance(_h67_gov_ui, pd.DataFrame) and not _h67_gov_ui.empty:
                    st.dataframe(_format_df(_h67_gov_ui), use_container_width=True, hide_index=True)
        ...
        st.caption("H74每天以全市場橫截面重排：真強勢加速、主流新鮮度、法人/成交資金加速度、TDCC週變化真相、訊號新鮮度與熟面孔慣性。只有當期TDCC持股、缺前期比較時一律標示『鎖碼未確認』。H74不建立Formal，H64/H63/H68權威不變。")
        _h74_engine_ok = bool(
            callable(apply_h74_fresh_mainstream_capital) and callable(build_h74_research_table)
            and callable(build_h74_decision_overview) and callable(build_h74_governance_summary)
            and H74_FRESH_CAPITAL_VERSION == H74_FRESH_CAPITAL_EXPECTED_VERSION
        )
        try:
            if _h74_engine_ok:
                _h51_source_ui = apply_h74_fresh_mainstream_capital(_h51_source_ui)
                _h74_ui = build_h74_decision_overview(_h51_source_ui, max_rows=15)
            else:
                _h74_ui = pd.DataFrame({"狀態":[f"H74新鮮主流資金未完整部署：{H74_FRESH_CAPITAL_VERSION}/{H74_FRESH_CAPITAL_EXPECTED_VERSION}"]})
        except Exception as _h74_ui_exc:
            _h74_ui = pd.DataFrame({"狀態":[f"H74決策總覽建立失敗：{type(_h74_ui_exc).__name__}: {_h74_ui_exc}"]})
        if isinstance(_h74_ui,pd.DataFrame) and not _h74_ui.empty:
            st.dataframe(_format_df(_h74_ui), use_container_width=True, hide_index=True)
        with st.expander("H74治理摘要｜主流輪動、TDCC真相與熟面孔治理", expanded=False):
            try:
                _h74_gov_ui = build_h74_governance_summary(_h51_source_ui) if _h74_engine_ok else pd.DataFrame()
                if isinstance(_h74_gov_ui,pd.DataFrame) and not _h74_gov_ui.empty:
                    st.dataframe(_format_df(_h74_gov_ui), use_container_width=True, hide_index=True)
            except Exception as _h74_gov_exc:
                st.caption(f"H74治理摘要暫時無法建立：{_h74_gov_exc}")

        # H73：補足H72「選得到、但強股不一定排前」的橫截面排序問題。
        render_pro_section("技術診斷｜H73 Leadership Breadth（非每日推薦榜）")
        st.caption("H73只做研究排序：族群廣度、領先加速、Regime轉折、模型一致性與分布矛盾。高動能但法人/大戶未確認會降序；族群共振且低分歧的領先股會上移。Formal仍由H64/H63，執行仍由H68決定。")
        _h73_engine_ok = bool(callable(apply_h73_leadership_breadth) and callable(build_h73_leadership_table) and callable(build_h73_governance_summary) and H73_LEADERSHIP_VERSION == H73_LEADERSHIP_EXPECTED_VERSION)
        try:
            if _h73_engine_ok:
                _h51_source_ui = apply_h73_leadership_breadth(_h51_source_ui)
                _h73_ui = build_h73_leadership_table(_h51_source_ui, max_rows=20, max_per_sector=3)
            else:
                _h73_ui = pd.DataFrame({"狀態":[f"H73領先共振未完整部署：{H73_LEADERSHIP_VERSION}/{H73_LEADERSHIP_EXPECTED_VERSION}"]})
        except Exception as _h73_ui_exc:
            _h73_ui = pd.DataFrame({"狀態":[f"H73領先共振建立失敗：{type(_h73_ui_exc).__name__}: {_h73_ui_exc}"]})
        if isinstance(_h73_ui,pd.DataFrame) and not _h73_ui.empty:
            st.dataframe(_format_df(_h73_ui), use_container_width=True, hide_index=True)
        with st.expander("H73治理摘要｜領先加速與分布矛盾", expanded=False):
            try:
                _h73_gov_ui = build_h73_governance_summary(_h51_source_ui) if _h73_engine_ok else pd.DataFrame()
                if isinstance(_h73_gov_ui,pd.DataFrame) and not _h73_gov_ui.empty:
                    st.dataframe(_format_df(_h73_gov_ui), use_container_width=True, hide_index=True)
            except Exception as _h73_gov_exc:
                st.caption(f"H73治理摘要暫時無法建立：{_h73_gov_exc}")

        # H72：把國內外成熟選股概念拆成8個可解釋模型，依市場Regime動態配權。
        render_pro_section("技術診斷｜H72 Multi-Model Alpha Ensemble（非每日推薦榜）")
        ...
        render_pro_section("超級AI觀察推薦｜H65 全市場多因子觀察雷達")
        st.caption("10大支柱：主流、趨勢、法人/外資/投信、大戶TDCC、營收、獲利/EPS、估值、技術買點、量能流動性、催化加速度；另計風險扣分與資料覆蓋率。Formal=0不再等於系統沒有研究價值。")
        _h65_engine_ok = bool(
            callable(apply_h65_multifactor_observation)
            and callable(build_h65_observation_radar_table)
            and callable(build_h65_indicator_coverage_table)
            and H65_OBSERVATION_VERSION == H65_OBSERVATION_EXPECTED_VERSION
        )
        try:
            if _h65_engine_ok:
                _h51_source_ui = apply_h65_multifactor_observation(_h51_source_ui)
                _h65_radar_ui = build_h65_observation_radar_table(_h51_source_ui, max_rows=20, max_per_sector=3)
                _h65_coverage_ui = build_h65_indicator_coverage_table(_h51_source_ui)
            else:
                _h65_radar_ui = pd.DataFrame({"狀態": [f"H65觀察雷達未完整部署：{H65_OBSERVATION_VERSION}/{H65_OBSERVATION_EXPECTED_VERSION}"]})
                _h65_coverage_ui = pd.DataFrame()
        except Exception as _h65_ui_exc:
            _h65_radar_ui = pd.DataFrame({"狀態": [f"H65觀察雷達建立失敗：{type(_h65_ui_exc).__name__}: {_h65_ui_exc}"]})
            _h65_coverage_ui = pd.DataFrame()
        if isinstance(_h65_radar_ui, pd.DataFrame) and not _h65_radar_ui.empty:
            _h65_tier_ui = _h65_radar_ui.get("H65觀察層級", pd.Series([""] * len(_h65_radar_ui))).fillna("").astype(str)
            _h65_cov_series = pd.to_numeric(_h65_radar_ui.get("H65資料覆蓋%", pd.Series([], dtype=float)), errors="coerce")
            render_pro_kpi_row([
                {"label": "W1重點觀察", "value": str(int(_h65_tier_ui.str.startswith("W1").sum())), "delta": "多因子高共振，仍非Formal"},
                {"label": "W2提前卡位", "value": str(int(_h65_tier_ui.str.startswith("W2").sum())), "delta": "相對優勢形成中"},
                {"label": "W3候選追蹤", "value": str(int(_h65_tier_ui.str.startswith("W3").sum())), "delta": "等待弱支柱改善"},
                {"label": "資料覆蓋中位", "value": (f"{float(_h65_cov_series.median()):.0f}%" if not _h65_cov_series.dropna().empty else "-"), "delta": "低覆蓋會自動降權"},
            ])
            st.dataframe(_format_df(_h65_radar_ui), use_container_width=True, hide_index=True)
        with st.expander("H65完整指標地圖與目前資料覆蓋｜查看還可以再接哪些指標", expanded=False):
            if isinstance(_h65_coverage_ui, pd.DataFrame) and not _h65_coverage_ui.empty:
                st.dataframe(_format_df(_h65_coverage_ui), use_container_width=True, hide_index=True)
            st.caption("H65會優先使用系統已有欄位；未接入的融資融券/借券、完整財務體質、更多事件催化等會列為擴充觀察層，不用缺值假裝高分。")
        ...
        try:
            ...
            _h51_sector_ui = pd.DataFrame({"狀態": [f"H51主流族群暫時無法建立：{_h51_sector_ui_exc}"]})
        ...
        render_pro_section("研究層｜H64核心候選｜真強勢×真主流×鎖碼趨勢")
        try:
            _h64_core_ui = build_h64_core_research_table(_h51_source_ui, max_rows=20) if callable(build_h64_core_research_table) else pd.DataFrame()
        except Exception as _h64_core_exc:
            _h64_core_ui = pd.DataFrame({"狀態": [f"H64核心候選暫時無法建立：{_h64_core_exc}"]})
        if isinstance(_h64_core_ui, pd.DataFrame) and not _h64_core_ui.empty:
            st.dataframe(_format_df(_h64_core_ui), use_container_width=True, hide_index=True)
        else:
            st.info("本輪沒有通過H64核心強勢/主流品質的候選；不拿舊Leader或熟面孔補位。")
        with st.expander("展開完整主流領漲研究證據（H51/H57，非第二份推薦清單）", expanded=False):
            try:
                _h51_leader_ui = build_h51_mainstream_leader_table(_h51_source_ui, max_rows=20) if callable(build_h51_mainstream_leader_table) else pd.DataFrame()
            except Exception as _h51_leader_ui_exc:
                _h51_leader_ui = pd.DataFrame({"狀態": [f"H51領漲股暫時無法建立：{_h51_leader_ui_exc}"]})
        ...
        render_pro_section("研究層｜股神推薦總排名｜H65多因子觀察＋H64核心真相＋H62/H61")
        st.caption("正式推薦仍由H64治理；非Formal研究排序則先看H65多因子觀察分與全市場百分位，再回看H64/H62/H61證據。這能提高新機會召回，同時避免把研究候選冒充買進清單。")
        ...
        if isinstance(master_rank, pd.DataFrame) and not master_rank.empty:
            ...
            render_pro_kpi_row([
                ...
                {"label": "H50主流可買", "value": format_number(top_row.get("H50推薦優先分"), 1), "delta": _safe_str(top_row.get("H50推薦決策")) or _safe_str(top_row.get("H50主流購買狀態"))},
                ...
                {"label": "前20名", "value": str(len(master_rank)), "delta": "H50新鮮主流×交易品質×族群分散"},
            ])
            master_cols = [c for c in [
                "股神推薦總排名", "股票代號", "股票名稱", "類別",
                "H50推薦優先分", "H50推薦等級", "H50推薦決策", "H50推薦許可", "H50推薦重複扣分", "H50推薦理由",
                "H50族群生命週期", "H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分", "H50波段機會階段",
                "H50主流購買優先分", "H50主流購買狀態", "H50重複推薦扣分", "H50主流購買理由",
                "H49上漲潛力分", "H49潛力等級", "H49潛力階段", "H49交易決策", "H49可執行分", "H49波段位置分", "H49延伸風險扣分", "H49上漲潛力理由",
        ...
        st.caption("只有H64品質覆核後仍為 EFFECTIVE-FORMAL 才是本輪真正正式推薦；即使列為正式推薦，也不是開盤市價直接買，仍須盤前、觸發、守價、停損與路徑RR成立。")
        if isinstance(_h63_formal_ui, pd.DataFrame) and "H63正式推薦順位" in _h63_formal_ui.columns and not _h63_formal_ui.empty:
            ...
            st.info("本輪沒有 H64 核心有效Formal；A-/Radar/前兆/FORMAL-QUALITY-HOLD都不是本輪正式推薦。空手或等待是有效決策。")
        ...
        if isinstance(battle, pd.DataFrame) and not battle.empty:
            ...
            st.caption("這裡是權威/條件/雷達的完整稽核，不是另一份推薦清單。只有 H63角色=F1/F0 且 H64有效權威=EFFECTIVE-FORMAL 才算正式推薦；A-、B+、Radar 都不是正式推薦。")
            try:
...
def _build_excel_bytes(
    ...
    if isinstance(summary_df, pd.DataFrame) and not summary_df.empty:
        ...
        elif formal_usable and len(formal_df) > 0:
            summary_df["空白/封鎖原因"] = f"無封鎖｜本輪有 {len(formal_df)} 檔上游Formal；H59唯一決策必須完整顯示並於盤前確認。"
        elif formal_usable and len(a_minus_df) > 0:
            summary_df["空白/封鎖原因"] = f"無Formal｜本輪有 {len(a_minus_df)} 檔A-準主推薦；只依權威限制等待，不得自行升格。"
        else:
            summary_df["空白/封鎖原因"] = "本輪沒有通過資料新鮮度與操作門檻的候選。"
    if isinstance(summary_df, pd.DataFrame) and not summary_df.empty:
        summary_df["H50Page07版本"] = PAGE07_SPEED_FIX_VERSION
        summary_df["H50主流生命週期引擎版本"] = H45_MAINSTREAM_VERSION
        summary_df["H50主流可買推薦引擎版本"] = H42_DUAL_ROUTE_VERSION
        summary_df["H51真人主線引擎版本"] = H51_HUMAN_MASTER_VERSION
        summary_df["H51版本一致性"] = "PASS" if (callable(apply_human_master_engine) and H51_HUMAN_MASTER_VERSION == H51_HUMAN_MASTER_EXPECTED_VERSION) else "FAIL｜請覆蓋H51真人主線引擎＋Page07"
        summary_df["H50版本一致性"] = "PASS" if (
        ...
        ) else "FAIL｜請重新覆蓋H50主流生命週期＋推薦引擎＋Page07"
        # 保留舊版診斷欄位供歷史Excel比對，不再作目前版本權威。
        summary_df["H47Page07版本"] = PAGE07_SPEED_FIX_VERSION
        summary_df["H47主流領漲引擎版本"] = H45_MAINSTREAM_VERSION
        summary_df["H49上漲潛力引擎版本"] = H42_DUAL_ROUTE_VERSION
    ...
    _h73_export_engine_ok = bool(
        callable(apply_h73_leadership_breadth) and callable(build_h73_leadership_table)
        and callable(build_h73_governance_summary)
        and H73_LEADERSHIP_VERSION == H73_LEADERSHIP_EXPECTED_VERSION
    )
    _h74_export_engine_ok = bool(
        callable(apply_h74_fresh_mainstream_capital) and callable(build_h74_research_table)
        and callable(build_h74_decision_overview) and callable(build_h74_excel_guide)
        and callable(build_h74_governance_summary)
        and H74_FRESH_CAPITAL_VERSION == H74_FRESH_CAPITAL_EXPECTED_VERSION
    )
    _h75_export_engine_ok = bool(
        callable(apply_h75_executive_decision) and callable(build_h75_today_ai_table)
        and callable(build_h75_sector_summary) and callable(build_h75_evidence_table)
        and callable(build_h75_performance_summary) and callable(build_h75_health_summary)
        and H75_EXECUTIVE_VERSION == H75_EXECUTIVE_EXPECTED_VERSION
    )
    _h76_export_engine_ok = bool(
    ...
    try:
        ...
        if callable(enrich_tdcc_holder_truth):
            try:
                _h60_export_source = enrich_tdcc_holder_truth(candidate_source, allow_network=False, timeout=2.0)
            except Exception:
                _h60_export_source = candidate_source
        ...
        if _h73_export_engine_ok:
            h51_source = apply_h73_leadership_breadth(h51_source)
        if _h74_export_engine_ok:
            h51_source = apply_h74_fresh_mainstream_capital(h51_source)
        if _h75_export_engine_ok:
        ...
        final_decision_df = build_h64_single_decision_truth_table(h51_source, max_rows=10) if _h51_export_engine_ok else pd.DataFrame({
            ...
            "修正方式": ["同時覆蓋 godpick_human_master_engine.py 與 pages/7_股神推薦.py 後重新部署。"],
        })
    ...
    try:
        h65_observation_df = build_h65_observation_radar_table(h51_source, max_rows=30, max_per_sector=3) if _h65_export_engine_ok else pd.DataFrame({"狀態": [f"H65多因子觀察雷達未完整部署：{H65_OBSERVATION_VERSION}/{H65_OBSERVATION_EXPECTED_VERSION}"]})
        h65_coverage_df = build_h65_indicator_coverage_table(h51_source) if _h65_export_engine_ok else pd.DataFrame({"狀態": ["H65指標覆蓋表暫時無法建立。"]})
    ...
    try:
        h67_priority_df = build_h67_priority_table(h51_source, max_rows=30, max_per_sector=2) if _h67_export_engine_ok else pd.DataFrame({"狀態": [f"H67次日治理未完整部署：{H67_GOVERNANCE_VERSION}/{H67_GOVERNANCE_EXPECTED_VERSION}"]})
        h67_governance_df = build_h67_governance_table(h51_source) if _h67_export_engine_ok else pd.DataFrame({"狀態": ["H67治理摘要暫時無法建立。"]})
    ...
    try:
        h73_leadership_df = build_h73_leadership_table(h51_source, max_rows=30, max_per_sector=3) if _h73_export_engine_ok else pd.DataFrame({"狀態": [f"H73領先共振未完整部署：{H73_LEADERSHIP_VERSION}/{H73_LEADERSHIP_EXPECTED_VERSION}"]})
        h73_governance_df = build_h73_governance_summary(h51_source) if _h73_export_engine_ok else pd.DataFrame({"狀態": ["H73治理摘要暫時無法建立。"]})
    except Exception as _h73_excel_exc:
        h73_leadership_df = pd.DataFrame({"狀態": [f"H73領先共振建立失敗：{type(_h73_excel_exc).__name__}: {_h73_excel_exc}"]})
        h73_governance_df = pd.DataFrame({"狀態": [f"H73治理摘要建立失敗：{type(_h73_excel_exc).__name__}: {_h73_excel_exc}"]})

    try:
        h74_overview_df = build_h74_decision_overview(h51_source, max_rows=15) if _h74_export_engine_ok else pd.DataFrame({"狀態": [f"H74決策總覽未完整部署：{H74_FRESH_CAPITAL_VERSION}/{H74_FRESH_CAPITAL_EXPECTED_VERSION}"]})
        h74_research_df = build_h74_research_table(h51_source, max_rows=30, max_per_sector=4) if _h74_export_engine_ok else pd.DataFrame({"狀態": ["H74新鮮主流資金暫時無法建立。"]})
        h74_guide_df = build_h74_excel_guide() if _h74_export_engine_ok else pd.DataFrame({"狀態": ["H74 Excel閱讀指南暫時無法建立。"]})
        h74_governance_df = build_h74_governance_summary(h51_source) if _h74_export_engine_ok else pd.DataFrame({"狀態": ["H74治理摘要暫時無法建立。"]})
    except Exception as _h74_excel_exc:
        h74_overview_df = pd.DataFrame({"狀態": [f"H74決策總覽建立失敗：{type(_h74_excel_exc).__name__}: {_h74_excel_exc}"]})
        h74_research_df = pd.DataFrame({"狀態": [f"H74研究表建立失敗：{type(_h74_excel_exc).__name__}: {_h74_excel_exc}"]})
        h74_guide_df = pd.DataFrame({"狀態": ["H74 Excel閱讀指南建立失敗。"]})
        h74_governance_df = pd.DataFrame({"狀態": [f"H74治理摘要建立失敗：{type(_h74_excel_exc).__name__}: {_h74_excel_exc}"]})
    ...
    _h62_hold_export = int(h51_source.get("H62有效權威", pd.Series([""] * len(h51_source), index=h51_source.index)).fillna("").astype(str).eq("FORMAL-HOLD").sum()) if isinstance(h51_source, pd.DataFrame) else 0
    _h62_shown_effective_export = int(final_decision_df.get("H62有效權威", pd.Series([""] * len(final_decision_df), index=final_decision_df.index)).fillna("").astype(str).eq("EFFECTIVE-FORMAL").sum()) if isinstance(final_decision_df, pd.DataFrame) else 0
    _h62_integrity_export = "PASS" if _h62_effective_formal_export == _h62_shown_effective_export else "FAIL"
    if isinstance(summary_df, pd.DataFrame) and not summary_df.empty:
        summary_df["H62原始Formal檔數"] = _h62_raw_formal_export
        summary_df["H62有效Formal檔數"] = _h62_effective_formal_export
        summary_df["H62FormalHold檔數"] = _h62_hold_export
        summary_df["H62唯一決策有效Formal顯示檔數"] = _h62_shown_effective_export
        summary_df["H62有效Formal完整性"] = f"{_h62_integrity_export}｜EffectiveFormal {_h62_shown_effective_export}/{_h62_effective_formal_export}"
        ...
        summary_df["H63正式推薦說明"] = "只有H64仍為EFFECTIVE-FORMAL才進正式推薦作戰；A-/Radar/每日條件候選均非正式推薦。"
        summary_df["H64核心真相版本"] = H64_CORE_VERSION
        if isinstance(h51_source, pd.DataFrame) and "H64有效權威" in h51_source.columns:
            _h64eff = h51_source["H64有效權威"].fillna("").astype(str)
            _h64tier = h51_source.get("H64研究層級", pd.Series([""] * len(h51_source), index=h51_source.index)).fillna("").astype(str)
            _h64lock = h51_source.get("H64鎖碼趨勢狀態", pd.Series([""] * len(h51_source), index=h51_source.index)).fillna("").astype(str)
            summary_df["H64核心有效Formal檔數"] = int(_h64eff.eq("EFFECTIVE-FORMAL").sum())
            summary_df["H64Formal品質暫停檔數"] = int(_h64eff.eq("FORMAL-QUALITY-HOLD").sum())
            summary_df["H64核心研究C2檔數"] = int(_h64tier.str.startswith("C2").sum())
            summary_df["H64待鎖碼C3檔數"] = int(_h64tier.str.startswith("C3").sum())
            summary_df["H64新主流E1檔數"] = int(_h64tier.str.startswith("E1").sum())
            summary_df["H64TDCC鎖碼已確認檔數"] = int(_h64lock.str.startswith(("LC", "LS")).sum())
            summary_df["H64TDCC鎖碼缺前期檔數"] = int(_h64lock.str.startswith("LU").sum())
        summary_df["H65多因子觀察版本"] = H65_OBSERVATION_VERSION
        if isinstance(h51_source, pd.DataFrame) and "H65觀察層級" in h51_source.columns:
            _h65tier_export = h51_source["H65觀察層級"].fillna("").astype(str)
            _h65cov_export = pd.to_numeric(h51_source.get("H65資料覆蓋%", pd.Series([], dtype=float)), errors="coerce")
            summary_df["H65_W1重點觀察檔數"] = int(_h65tier_export.str.startswith("W1").sum())
            summary_df["H65_W2提前卡位檔數"] = int(_h65tier_export.str.startswith("W2").sum())
            summary_df["H65_W3候選追蹤檔數"] = int(_h65tier_export.str.startswith("W3").sum())
        ...
        summary_df["H67次日治理版本"] = H67_GOVERNANCE_VERSION
        if isinstance(h51_source, pd.DataFrame) and "H67研究優先層級" in h51_source.columns:
            _h67tier_export = h51_source["H67研究優先層級"].fillna("").astype(str)
            _h67mkt_export = pd.to_numeric(h51_source.get("H67市場Regime調整", pd.Series([], dtype=float)), errors="coerce")
            _h67sec_export = pd.to_numeric(h51_source.get("H67族群資金調整", pd.Series([], dtype=float)), errors="coerce")
            summary_df["H67_P1次日優先檔數"] = int(_h67tier_export.str.startswith("P1").sum())
            summary_df["H67_P2次日次優先檔數"] = int(_h67tier_export.str.startswith("P2").sum())
            summary_df["H67_C1條件觀察檔數"] = int(_h67tier_export.str.startswith("C1").sum())
        ...
        summary_df["H73領先共振版本"] = H73_LEADERSHIP_VERSION
        ...
        if isinstance(h74_governance_df, pd.DataFrame) and not h74_governance_df.empty:
            ...
            if not _h74_unconfirmed.empty:
                summary_df["H74TDCC缺前期比較檔數"] = _h74_unconfirmed.iloc[0]
        if isinstance(h51_source, pd.DataFrame) and "H73研究層級" in h51_source.columns:
            _h73tier_export = h51_source["H73研究層級"].fillna("").astype(str)
            summary_df["H73_L1領先共振檔數"] = int(_h73tier_export.str.startswith("L1").sum())
            summary_df["H73_L2領先觀察檔數"] = int(_h73tier_export.str.startswith("L2").sum())
            summary_df["H73_L3候選追蹤檔數"] = int(_h73tier_export.str.startswith("L3").sum())
            summary_df["H73Regime轉折狀態"] = _safe_str(h51_source.iloc[0].get("H73Regime轉折狀態"))
            summary_df["H73分布矛盾中位扣分"] = round(float(pd.to_numeric(h51_source.get("H73分布矛盾扣分", pd.Series([],dtype=float)),errors="coerce").median()),2) if not pd.to_numeric(h51_source.get("H73分布矛盾扣分", pd.Series([],dtype=float)),errors="coerce").dropna().empty else None
            summary_df["H73權威邊界"] = "L1/L2/L3皆為研究排序；不得建立Formal，也不得解除H68執行否決。"
        ...
        if isinstance(h51_source, pd.DataFrame) and "H60鎖碼來源" in h51_source.columns:
            ...
            _h60_proxy = int(h51_source["H60鎖碼來源"].fillna("").astype(str).str.startswith("PROXY").sum())
            summary_df["H60真實大戶鎖碼覆蓋檔數"] = _h60_actual
            summary_df["H60代理鎖碼檔數"] = _h60_proxy
            summary_df["H60大戶鎖碼真相"] = f"ACTUAL={_h60_actual}｜PROXY={_h60_proxy}；PROXY不得視為真實千張大戶"
    try:
        h51_leader_df = build_h51_mainstream_leader_table(h51_source, max_rows=20) if callable(build_h51_mainstream_leader_table) else pd.DataFrame()
    except Exception as _h51_leader_exc:
        h51_leader_df = pd.DataFrame({"狀態": [f"H51主流領漲表建立失敗：{type(_h51_leader_exc).__name__}: {_h51_leader_exc}"]})
    try:
        h51_sector_df = build_h51_sector_table(h51_source, max_rows=15) if callable(build_h51_sector_table) else pd.DataFrame()
    except Exception as _h51_sector_exc:
        h51_sector_df = pd.DataFrame({"狀態": [f"H51主流族群表建立失敗：{type(_h51_sector_exc).__name__}: {_h51_sector_exc}"]})

    try:
        # H51總排名必須以同一份H51母體重建，避免第一張與總排名是兩套邏輯。
        master_rank_df = _phase90_build_master_recommendation_rank(h51_source, top_n=30)
    except Exception:
        pass
    # H51：不再建立未匯出的舊 V188/導航中間表，降低完整 Excel 建立時間。
    try:
        v188_truth_df = pd.DataFrame(load_t1_truth_rows(limit=500) if callable(load_t1_truth_rows) else [])
    except Exception:
        v188_truth_df = pd.DataFrame()
    try:
        _truth_sum = load_t1_truth_summary() if callable(load_t1_truth_summary) else {}
        _perf_rows = []
        if isinstance(_truth_sum, dict):
            for _kpi in [
                "matured_t1_samples", "executable_samples", "executable_win_rate_pct", "avg_selection_alpha_pct",
                "H49高潛力成熟樣本", "H49高潛力正報酬率%", "H49高潛力平均1日報酬%", "H49高潛力平均SelectionAlpha%",
                "H50主流推薦成熟樣本", "H50主流推薦正報酬率%", "H50主流推薦平均1日報酬%", "H50主流推薦平均SelectionAlpha%",
                "H51專業主線成熟樣本", "H51專業主線正報酬率%", "H51專業主線平均1日報酬%", "H51專業主線平均SelectionAlpha%",
                "H57_E1成熟樣本", "H57_E1正報酬率%", "H57_E1平均1日報酬%", "H57_E1平均SelectionAlpha%",
                "H57_PI3成熟樣本", "H57_PI3正報酬率%", "H57_PI3平均1日報酬%", "H57_PI3平均SelectionAlpha%",
                "H59_A1成熟樣本", "H59_A1正報酬率%", "H59_A1平均1日報酬%", "H59_A1平均SelectionAlpha%",
                "H59_A0成熟樣本", "H59_A0正報酬率%", "H59_A0平均1日報酬%", "H59_A0平均SelectionAlpha%",
                "H60_MR1成熟樣本", "H60_MR1正報酬率%", "H60_MR1平均1日報酬%", "H60_MR1平均SelectionAlpha%",
                "H60_SB1成熟樣本", "H60_SB1正報酬率%", "H60_SB1平均1日報酬%", "H60_SB1平均SelectionAlpha%",
                ...
                "H73學習快照成熟樣本", "H73學習啟用狀態",
                "H73_L1成熟樣本", "H73_L1正報酬率%", "H73_L1平均1日報酬%", "H73_L1平均SelectionAlpha%",
                "H73_L2成熟樣本", "H73_L2正報酬率%", "H73_L2平均1日報酬%", "H73_L2平均SelectionAlpha%",
                "H73_L3成熟樣本", "H73_L3正報酬率%", "H73_L3平均1日報酬%", "H73_L3平均SelectionAlpha%",
                "H73排名成熟交易日", "H73平均RankIC", "H73平均NDCG@10",
                "H74學習快照成熟樣本", "H74學習啟用狀態",
                "H74_F1成熟樣本", "H74_F1正報酬率%", "H74_F1平均1日報酬%", "H74_F1平均SelectionAlpha%",
                "H74_F2成熟樣本", "H74_F2正報酬率%", "H74_F2平均1日報酬%", "H74_F2平均SelectionAlpha%",
                "H74_F3成熟樣本", "H74_F3正報酬率%", "H74_F3平均1日報酬%", "H74_F3平均SelectionAlpha%",
                "H74排名成熟交易日", "H74平均RankIC", "H74平均NDCG@10",
                "brier_score", "brier_skill_vs_base_rate_pct",
            ]:
                if _kpi in _truth_sum:
                    _perf_rows.append({"績效指標": _kpi, "目前數值": _truth_sum.get(_kpi)})
        ai_performance_df = pd.DataFrame(_perf_rows) if _perf_rows else pd.DataFrame({"狀態": ["績效真相尚未建立或永久權威尚未恢復。"]})
    except Exception as _h51_perf_exc:
        ai_performance_df = pd.DataFrame({"狀態": [f"AI績效摘要建立失敗：{type(_h51_perf_exc).__name__}: {_h51_perf_exc}"]})

    # H51：已取消未匯出的舊攻略/雷達中間表，只保留總排名欄位版型。
    master_rank_df = _apply_excel_export_column_order(
        master_rank_df, "excel_master_rank", _get_master_rank_default_cols()
    )

    # H51：預設Excel只保留真正有決策價值的核心活頁。
    # 其他雷達/診斷不再重複匯出；完整研究欄位仍保留於「股神推薦總排名」。
    health_rows = []
    try:
        if isinstance(summary_df, pd.DataFrame) and not summary_df.empty:
            for c in summary_df.columns:
                health_rows.append({"類型": "推薦摘要", "項目": c, "數值": summary_df.iloc[0].get(c)})
        if isinstance(scan_df, pd.DataFrame) and not scan_df.empty:
            for c in scan_df.columns:
                health_rows.append({"類型": "資料健康", "項目": c, "數值": scan_df.iloc[0].get(c)})
    except Exception:
        pass
    health_df = pd.DataFrame(health_rows) if health_rows else pd.DataFrame({"狀態": ["目前沒有資料健康摘要。"]})

    # H75：主管日常只需要前五張；其餘完整模型/診斷頁保留但預設隱藏。
    try:
        h75_today_df = build_h75_today_ai_table(h51_source, max_rows=12, max_per_sector=3) if _h75_export_engine_ok else h74_overview_df.copy()
        h75_sector_df = build_h75_sector_summary(h51_sector_df, max_rows=10) if _h75_export_engine_ok else h51_sector_df.copy()
        h75_evidence_df = build_h75_evidence_table(h51_source, max_rows=20) if _h75_export_engine_ok else h74_research_df.copy()
        h75_perf_df = build_h75_performance_summary(ai_performance_df) if _h75_export_engine_ok else ai_performance_df.copy()
        h75_health_df = build_h75_health_summary(health_df, max_rows=30) if _h75_export_engine_ok else health_df.copy()
    except Exception as _h75_excel_exc:
        _h75_msg = f"H75主管決策表建立失敗：{type(_h75_excel_exc).__name__}: {_h75_excel_exc}"
        h75_today_df = pd.DataFrame({"狀態": [_h75_msg]})
        h75_sector_df = h51_sector_df.copy() if isinstance(h51_sector_df, pd.DataFrame) else pd.DataFrame({"狀態": [_h75_msg]})
        h75_evidence_df = h74_research_df.copy() if isinstance(h74_research_df, pd.DataFrame) else pd.DataFrame({"狀態": [_h75_msg]})
    ...
    try:
        if not callable(build_h78_tables):
            raise RuntimeError(H78_IMPORT_ERROR or "H78引擎未部署")
        _h78_export = build_h78_tables(candidate_source)
    except Exception as _h78_error:
        _h78_failure = pd.DataFrame({"狀態":[f"H78決策失敗，非沒有推薦：{_h78_error}"]})
        _h78_export = {k:_h78_failure for k in ["recommendations","waiting","audit","health"]}

    sheets = [
        ("01_今日推薦與計畫", _h78_export["recommendations"], "H78沒有符合選股政策的候選；請查完整原因與資料健康。"),
        ("02_等待與核心監控", _h78_export["waiting"], "目前沒有追價/證據等待或結構核心監控股。"),
        ("03_今日主流資金", h75_sector_df, "目前沒有可用主流族群資料。"),
        ("04_驗證與風險證據", _h78_export["audit"], "目前沒有足夠的驗證增量/風險證據。"),
        ("05_績效煞車與健康", pd.concat([_h78_export["health"], h77_perf_health_df], ignore_index=True), "目前沒有成熟績效/系統健康資料。"),
        ...
        ("H74新鮮主流資金", h74_research_df, "H74用真強勢/主流新鮮/法人與成交資金/TDCC週變化/重複慣性重排研究順位；F1/F2/F3均非Formal。"),
        ("H74治理摘要", h74_governance_df, "H74主流輪動、TDCC趨勢真相與熟面孔治理摘要。"),
        ("超級AI最終決策", final_decision_df, "H64強勢/主流/鎖碼Formal真相佐證；H74已成為每日第一研究總覽。"),
        ("H73領先共振", h73_leadership_df, "H73用族群廣度/領先加速/Regime轉折/分布矛盾改善研究排序；L1/L2/L3均非Formal。"),
        ("H73治理摘要", h73_governance_df, "H73領先排序與分布矛盾治理摘要。"),
        ...
        ("AI績效驗證", ai_performance_df, "目前沒有成熟績效驗證資料。"),
        ("T+1實戰真相", v188_truth_df, "尚無成熟 T+1 實戰真相；待下一交易日更新。"),
        ("系統健康與資料品質", health_df, "目前沒有系統健康/資料品質資料。"),
    ]
    ...
    for sheet_name, frame, empty_message in sheets:
        ...
        diag_rows.append({
            "分頁": sheet_name,
            "用途": ("日常第一張｜驗證增量研究優先/正式權威" if sheet_name == "01_今日推薦與計畫" else "日常第二張｜追價/證據等待與結構核心" if sheet_name == "02_等待與核心監控" else "日常第三張｜主流族群與資金輪動" if sheet_name == "03_今日主流資金" else "日常第四張｜增量覆蓋/追價/進場風險" if sheet_name == "04_驗證與風險證據" else "日常第五張｜績效煞車＋資料健康" if sheet_name == "05_績效煞車與健康" else "技術診斷｜預設隱藏"),
            "列數": len(frame) if isinstance(frame, pd.DataFrame) else 0,
            "欄數": len(frame.columns) if isinstance(frame, pd.DataFrame) else 0,
        })

    _h77_visible_sheets = {"01_今日推薦與計畫", "02_等待與核心監控", "03_今日主流資金", "04_驗證與風險證據", "05_績效煞車與健康"}
    ...
    try:
        wb.active = wb.sheetnames.index("01_今日推薦與計畫")
    except Exception:
        pass
    wb.save(output)
    output.seek(0)
...
def _render_export_block(rec_df: pd.DataFrame, category_strength_df: pd.DataFrame, top_n: int):
    ...
    _show_excel_layout_manager = st.toggle(
        ...
        help="開啟後可直接用欄位名稱調整股神推薦總排名欄位順序；不需要輸入順位或反覆左右移動，設定會永久保存。",
    )
...
def _run_page07_automation_v191_h2(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        # H62: Page07 executes, but only this-run EFFECTIVE-FORMAL may enter the
        # formal persistence path. Raw Formal / FORMAL-HOLD remains audit data.
        try:
...
def main():
    ...
    render_pro_info_card(
        ...
        [
            ...
            ("分層績效資料庫", "正式/A-/R1維持推薦紀錄；近門檻與漏選強勢另存影子校正樣本。近門檻未達成熟證據前不直接調權，避免單日績效把門檻錯誤放寬。", ""),
    ...
    st.warning("這張表保留是為了勾選、自選股與推薦紀錄同步；它仍含舊版B+/A-/雷達角色，不能拿來取代上方H62唯一決策。")
    ...
    if top_apply_selection:
        ...
        st.success(f"已套用舊版作戰名單勾選：{len(picked_codes_from_top)} 檔；此動作只做同步/紀錄，不改變H62唯一決策。")
    else:
    ...
    if _safe_str(st.session_state.get(_k("pick_strategy"), "結合版")) == "結合版" and isinstance(hot_pick_df, pd.DataFrame) and not hot_pick_df.empty:
        ...
        st.caption("這是H57之前的舊版補抓研究表，不是H60決策來源；真正提前發現請優先看H57 E1/PI3與H62唯一決策。")
        hot_show_cols = [
