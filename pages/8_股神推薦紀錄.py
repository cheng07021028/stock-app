...
GODPICK_RECORD_COLUMNS = [
    ...
    "推薦標籤", "推薦理由摘要", "推薦價格", "近端支撐", "近端壓力", "突破確認價", "停損參考", "停損價", "賣出目標1", "賣出目標2", "推薦日期", "推薦時間",
...
def _ensure_godpick_record_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = [
        "推薦總分", "族群資金流分數", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "技術結構分數", "起漲前兆分數", "機會股分數", "低檔位置分數", "拉回承接分數", "支撐回測分數", "止跌轉強分數", "進場時機分數", "近端支撐", "主要支撐", "近端壓力", "突破確認價", "停損參考", "風險報酬比_決策", "追高風險分數_決策", "飆股起漲分數", "交易可行分數", "類股熱度分數", "強勢族群等級", "族群資金流分數", "族群輪動狀態", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "族群策略建議", "族群資金流說明", 
    ...
    with tabs[0]:
            ...
            with st.expander("欄位順序管理", expanded=True):
                ...
                with preset_cols[0]:
                    if st.button("方案A：交易核心", use_container_width=True):
                        preset = [c for c in [
                            "record_id", "股票代號", "股票名稱", "推薦模式", "推薦等級", "進場時機", "進場時機分數", "建議動作", "等待條件", "操作區間", "近端支撐", "近端壓力", "突破確認價", "停損參考", "追高風險等級", "是否建議追價", "推薦價格", "最新價",
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "類股熱度分數": st.column_config.NumberColumn("類股熱度分數", "強勢族群等級", "族群資金流分數", "族群輪動狀態", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "族群策略建議", "族群資金流說明",  "強勢族群等級", "族群資金流分數", "族群輪動狀態", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "族群策略建議", "族群資金流說明", format="%.2f", disabled=True),
