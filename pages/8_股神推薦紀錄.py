...
GODPICK_RECORD_COLUMNS = [
  ...
  "上漲機率估計%", "上漲機率等級", "上漲機率信心", "上漲機率說明", "上漲機率因子明細", 
    ...
    "推薦批次日期", "推薦批次時間", "推薦執行ID", "推薦執行來源", "推薦觸發方式", "推薦執行版本", "原始推薦日期",
    ...
    "推薦後1日%", "推薦後3日%", "推薦後5日%", "推薦後10日%", "推薦後20日%", "推薦後最大漲幅%", "推薦後最大回撤%", "是否曾達標_回測", "達標確認狀態", "回測事件摘要", "是否達標_回測", "是否停損_回測", "命中結果", "績效評語", "追蹤更新時間", "進場觸發狀態", "進場觸發日期", "進場評估路徑", "是否納入可執行績效", "執行基準價", "理論進場參考價", "執行價來源", "執行價可成交驗證", "觸發訊號品質分", "觸發後收盤績效%", "觸發當日收盤績效%", "觸發當日最高報酬%", "觸發當日最大回撤%", "觸發當日收盤保留率%", "觸發收盤確認層級", "隔日候選漲跌%", "隔日執行命中結果", "隔日績效檢討標籤", "未觸發漏選標記", "候選與交易分流說明", "績效更新版本", "可執行交易1日%", "可執行交易3日%", "可執行交易5日%", "可執行交易10日%", "可執行交易20日%", "可執行交易最大漲幅%", "可執行交易最大回撤%", "除權息調整旗標", "績效計算口徑", "3日績效%", "5日績效%", "10日績效%", "20日績效%",
]
...
def _ensure_godpick_record_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = _dedupe_keep_order_v73(list(V73_NUMERIC_FIELDS) + ["理論進場參考價"])
    ...
    text_cols = [
        ...
        "推薦分桶", "起漲等級", "信心等級", "推薦日期", "推薦時間", "推薦批次日期", "推薦批次時間", "推薦執行ID", "推薦執行來源", "推薦觸發方式", "推薦執行版本", "原始推薦日期", "建立時間", "更新時間", "最新更新時間", "模式績效標籤", "股神建議動作", "股神信心", "股神進場區間", "股神推論", "績效資料型態", "績效資料來源", "執行價來源", "備註",
    ]
