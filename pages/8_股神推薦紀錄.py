...
PAGE_TITLE = "股神推薦紀錄｜v82 Duplicate Columns 修正版"
PFX = "godpick_record_"
GOD_DECISION_V10_LINK_VERSION = "record_v10_entry_decision_v1_20260428"
BACKTEST_V12_VERSION = "record_v82_duplicate_columns_fix_20260430"
    ...
    with st.expander(title, expanded=True):
        ...
        with tabs_v50[4]:
            ...
            if weak_col:
                ...
                # v82：避免 pyarrow 因欄位清單重複而報 Duplicate column names。
                raw_cols = ["股票代號", "股票名稱", "類別", "推薦模式", "推薦等級", "推薦總分", "上漲機率估計%", "上漲機率等級", "上漲機率信心", "上漲機率說明", "上漲機率因子明細", weak_col, "推薦後最大回撤%", "命中結果", "績效評語", "推薦日期", "推薦理由摘要", "風險說明"]
                cols = []
                for c in raw_cols:
                    if c in weak.columns and c not in cols:
                        cols.append(c)
                weak_show = weak.loc[:, cols].copy()
                weak_show = weak_show.loc[:, ~weak_show.columns.duplicated()].copy()
                st.dataframe(weak_show, use_container_width=True, hide_index=True)
            else:
