...
def _build_candlestick_chart(df: pd.DataFrame, stock_label: str, show_ma: bool, show_pivots: bool, peak_idx: tuple[int, ...], trough_idx: tuple[int, ...]) -> go.Figure:
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            hovertemplate=(
                ...
                "振幅：%{customdata[7]:.2f}%<extra></extra>"
            ),
        ),
        row=1,
        col=1,
    ...
    if has_volume:
        ...
        fig.add_trace(
            go.Bar(
                ...
                hovertemplate="<b>成交量｜%{x|%Y-%m-%d}</b><br>成交量：%{y:,.0f}<extra></extra>",
            ),
            row=2,
            col=1,
        )
            ...
            if col in work.columns:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        connectgaps=False,
                        hovertemplate=f"<b>{col}｜%{{x|%Y-%m-%d}}</b><br>{col}：%{{y:,.2f}}<extra></extra>",
                    ),
                    row=1,
                    col=1,
                )
            ...
            if idxs:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate="<b>起漲訊號｜%{x|%Y-%m-%d}</b><br>低點：%{customdata:,.2f}<extra></extra>",
            ...
            if idxs:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate="<b>起跌訊號｜%{x|%Y-%m-%d}</b><br>高點：%{customdata:,.2f}<extra></extra>",
    ...
    fig.update_xaxes(
        ...
        spikemode="across",
        spikesnap="cursor",
        spikecolor="rgba(59,130,246,0.28)",
        unifiedhovertitle=dict(text="<b>%{x|%Y-%m-%d}</b>"),
        spikethickness=1,
        showgrid=False,
        tickformat="%Y-%m-%d",
        row=1,
