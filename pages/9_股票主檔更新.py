...
def _build_final_god_score_row(row: pd.Series, mode: str, market_score: float) -> tuple[float, str]:
    ...
    backtest_score = _safe_float(row.get("買點回測分數"), 0) or 0

    if mode == "飆股模式":
        total = market_score * 0.10 + category_heat_score * 0.14 + pattern_score * 0.18 + burst_score * 0.16 + prelaunch_score * 0.14 + technical_score * 0.09 + trade_score * 0.07 + backtest_score * 0.12
        tag = "爆發優先 / 起漲優先 / 回測驗證"
    elif mode == "波段模式":
        total = market_score * 0.12 + technical_score * 0.20 + category_heat_score * 0.16 + trade_score * 0.12 + pattern_score * 0.10 + factor_score * 0.10 + risk_score * 0.06 + backtest_score * 0.14
        tag = "趨勢延續 / 波段優先 / 回測驗證"
    elif mode == "領頭羊模式":
        total = market_score * 0.10 + leader_advantage * 0.20 + category_heat_score * 0.20 + technical_score * 0.14 + prelaunch_score * 0.08 + pattern_score * 0.08 + trade_score * 0.08 + backtest_score * 0.12
        tag = "類股領先 / 龍頭優先 / 回測驗證"
    else:
        total = market_score * 0.12 + technical_score * 0.16 + prelaunch_score * 0.12 + category_heat_score * 0.14 + factor_score * 0.10 + trade_score * 0.10 + pattern_score * 0.08 + burst_score * 0.06 + backtest_score * 0.12
        tag = "綜合推薦 / 回測驗證"
...
def _build_recommend_reason_v2(r: pd.Series) -> str:
    ...
    if _safe_float(r.get("爆發力分數"), 0) >= 75:
        parts.append("爆發力佳")
    if _safe_float(r.get("買點回測分數"), 0) >= 80:
        parts.append(f"回測{_safe_str(r.get('買點分級'))}級")
    if _safe_float(r.get("20日勝率%"), 0) >= 65:
        parts.append("20日勝率高")
    if _safe_float(r.get("風險分數"), 0) < 60:
        parts.append("風險需控管")
    text = "、".join([x for x in parts if x][:7])
    if not text:
        text = "結構偏多，列入觀察"
    ...
    bt = _safe_str(r.get("買點分級"))
    win20 = format_number(r.get("20日勝率%"), 1) if pd.notna(r.get("20日勝率%")) else "—"
    return f"{text}｜切入區 {entry_zone or '—'}｜停損 {stop_loss}｜目標1 {target_1}｜回測 {bt or '—'} / 20日勝率 {win20}%"
...
def _backtest_grade_label(score: float) -> str:
...
def _build_buypoint_backtest(df: pd.DataFrame, signal_snapshot: dict, sr_snapshot: dict) -> dict[str, Any]:
    cols = [
        "buy_signal", "buy_grade", "buy_score",
        "ret_5d", "ret_10d", "ret_20d",
    ]
    empty_detail = pd.DataFrame(columns=cols)
    ...
    vol5 = pd.to_numeric(work.get("VOL5"), errors="coerce")
    vol20 = pd.to_numeric(work.get("VOL20"), errors="coerce")
    ma20 = pd.to_numeric(work.get("MA20"), errors="coerce")
    ma60 = pd.to_numeric(work.get("MA60"), errors="coerce")
    dif = pd.to_numeric(work.get("DIF"), errors="coerce")
    dea = pd.to_numeric(work.get("DEA"), errors="coerce")
    k = pd.to_numeric(work.get("K"), errors="coerce")
    d = pd.to_numeric(work.get("D"), errors="coerce")
    ret5 = pd.to_numeric(work.get("RET5"), errors="coerce")
    atr14 = pd.to_numeric(work.get("ATR14"), errors="coerce")

    work["res_20_roll"] = high.shift(1).rolling(20).max()
    work["sup_20_roll"] = low.shift(1).rolling(20).min()
    work["vol_ratio"] = vol5 / vol20.replace(0, pd.NA)
    work["atr_pct"] = atr14 / close.replace(0, pd.NA) * 100
    work["bias_ma20"] = (close - ma20) / ma20.replace(0, pd.NA) * 100

    score = pd.Series(30.0, index=work.index, dtype=float)
    score += ((close >= ma20) & ma20.notna()).astype(float) * 12
    score += ((close >= ma60) & ma60.notna()).astype(float) * 8
    score += ((ma20.diff(3) > 0) & ma20.notna()).astype(float) * 10
    score += ((ma60.diff(3) > 0) & ma60.notna()).astype(float) * 6
    score += ((k > d) & k.notna() & d.notna()).astype(float) * 8
    score += ((k.shift(1) <= d.shift(1)) & (k > d) & k.notna() & d.notna()).astype(float) * 10
    score += ((dif > dea) & dif.notna() & dea.notna()).astype(float) * 8
    score += ((dif.shift(1) <= dea.shift(1)) & (dif > dea) & dif.notna() & dea.notna()).astype(float) * 8
    score += ((work["vol_ratio"] >= 1.15) & work["vol_ratio"].notna()).astype(float) * 10
    score += ((work["vol_ratio"] >= 1.5) & work["vol_ratio"].notna()).astype(float) * 6
    score += ((ret5 >= 0) & ret5.notna()).astype(float) * 4

    dist_break = (work["res_20_roll"] - close) / work["res_20_roll"].replace(0, pd.NA) * 100
    dist_support = (close - work["sup_20_roll"]) / work["sup_20_roll"].replace(0, pd.NA) * 100
    score += (((dist_break >= -1.5) & (dist_break <= 3.0)) | ((dist_support >= 0) & (dist_support <= 4.0))).astype(float) * 10

    score -= ((work["bias_ma20"] >= 10) & work["bias_ma20"].notna()).astype(float) * 10
    score -= ((work["atr_pct"] > 9) & work["atr_pct"].notna()).astype(float) * 10
    score -= ((ret5 > 18) & ret5.notna()).astype(float) * 8

    work["buy_score"] = score.apply(lambda x: _score_clip(x))
    work["buy_grade"] = work["buy_score"].apply(_backtest_grade_label)
    work["buy_signal"] = (
        (work["buy_score"] >= 70)
        & close.notna()
        & ma20.notna()
        & (close >= ma20)
        & ((work["vol_ratio"].fillna(0) >= 1.0) | ((dist_break >= -1.5) & (dist_break <= 3.0)))
    )

    for n in [5, 10, 20]:
        work[f"ret_{n}d"] = (close.shift(-n) / close - 1) * 100

    detail = work.loc[work["buy_signal"], ["日期", "buy_signal", "buy_grade", "buy_score", "ret_5d", "ret_10d", "ret_20d"]].copy()
    detail = detail.dropna(subset=["ret_5d", "ret_10d", "ret_20d"], how="all").reset_index(drop=True)

    signal_count = len(detail)
    if signal_count <= 0:
        current_score = _safe_float(signal_snapshot.get("score"), 0) or 0
        structure = _safe_str(sr_snapshot.get("break_signal", ("區間內", ""))[0] if isinstance(sr_snapshot.get("break_signal"), tuple) else sr_snapshot.get("break_signal", "區間內"))
        fallback_score = _score_clip(50 + current_score * 6 + (10 if "突破" in structure else 0))
        grade = _backtest_grade_label(fallback_score)
        return {
    ...
    composite = 0.0
    composite += (win5 or 0) * 0.20
    composite += (win10 or 0) * 0.30
    composite += (win20 or 0) * 0.30
    composite += max(min((avg5 or 0) * 2.2, 12), -12)
    composite += max(min((avg10 or 0) * 2.0, 14), -14)
    composite += max(min((avg20 or 0) * 1.8, 16), -16)
    composite += min(signal_count, 12) * 1.0
    composite = _score_clip(composite)
    grade = _backtest_grade_label(composite)

    recent_row = detail.iloc[-1] if not detail.empty else None
    recent_date = ""
    recent_score = None
    recent_grade = ""
    if recent_row is not None:
        try:
            recent_date = pd.to_datetime(recent_row["日期"]).strftime("%Y-%m-%d")
        except Exception:
            recent_date = _safe_str(recent_row.get("日期"))
        recent_score = _safe_float(recent_row.get("buy_score"))
        recent_grade = _safe_str(recent_row.get("buy_grade"))

    summary = f"{grade}級｜樣本 {signal_count} 次｜5/10/20日勝率 {format_number(win5,1)}% / {format_number(win10,1)}% / {format_number(win20,1)}%"
    return {
        "score": composite,
        "grade": grade,
        "signal_count": signal_count,
        "recent_signal_date": recent_date,
        "recent_signal_score": recent_score,
        "recent_signal_grade": recent_grade,
        "win_rate_5d": win5,
        "win_rate_10d": win10,
        "win_rate_20d": win20,
        "avg_return_5d": avg5,
...
def _analyze_stock_bundle(stock_no: str, stock_name: str, market_type: str, start_dt: date, end_dt: date, risk_strictness: str) -> dict[str, Any]:
    try:
        ...
        backtest_buy = _build_buypoint_backtest(hist_df, signal_snapshot, sr_snapshot)
...
def _analyze_one_stock_for_recommend(
    ...
    return {
        ...
        "row": {
            ...
            "交易可行分數": trade_score,
            "買點回測分數": _safe_float(bundle["backtest_buy"].get("score"), 0) or 0,
            "買點分級": _safe_str(bundle["backtest_buy"].get("grade")),
            "回測樣本數": int(bundle["backtest_buy"].get("signal_count", 0) or 0),
            "最近買點日期": _safe_str(bundle["backtest_buy"].get("recent_signal_date")),
            "最近買點分級": _safe_str(bundle["backtest_buy"].get("recent_signal_grade")),
            "最近買點分數": _safe_float(bundle["backtest_buy"].get("recent_signal_score")),
            "5日勝率%": _safe_float(bundle["backtest_buy"].get("win_rate_5d")),
            "10日勝率%": _safe_float(bundle["backtest_buy"].get("win_rate_10d")),
            "20日勝率%": _safe_float(bundle["backtest_buy"].get("win_rate_20d")),
            "5日平均報酬%": _safe_float(bundle["backtest_buy"].get("avg_return_5d")),
            "10日平均報酬%": _safe_float(bundle["backtest_buy"].get("avg_return_10d")),
            "20日平均報酬%": _safe_float(bundle["backtest_buy"].get("avg_return_20d")),
            "買點回測摘要": _safe_str(bundle["backtest_buy"].get("summary")),
...
def _build_recommend_df(
    ...
    worker_count = min(16, max(4, total_count // 10 if total_count >= 10 else 4))
...
def _format_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pct_cols = ["區間漲跌幅%", "20日壓力距離%", "20日支撐距離%", "類股平均漲幅", "3日績效%", "5日績效%", "10日績效%", "20日績效%", "5日勝率%", "10日勝率%", "20日勝率%", "5日平均報酬%", "10日平均報酬%", "20日平均報酬%"]
    score_cols = [
        "訊號分數", "雷達均分", "技術結構分數", "起漲前兆分數", "交易可行分數", "買點回測分數", "最近買點分數",
...
def main():
    ...
    render_pro_hero(
        title="股神推薦｜V5 完整整合加速版",
        subtitle="整合 V4 加速記憶版 + 可回測買點分級版；保留功能並加快顯示。",
    )
    ...
    render_pro_info_card(
        ...
        [
            ...
            ("買點回測", "新增 S/A/B/C/D 買點分級，並回看 5 / 10 / 20 日勝率與平均報酬。", ""),
    ...
    if focus_row is not None:
        render_pro_info_card(
            ...
            [
                ...
                ("買點分級", _safe_str(focus_row.get("買點分級")), ""),
                ("買點回測分數", format_number(focus_row.get("買點回測分數"), 1), ""),
                ("回測樣本數", format_number(focus_row.get("回測樣本數"), 0), ""),
                ("5日 / 10日 / 20日勝率", f"{format_number(focus_row.get('5日勝率%'),1)}% / {format_number(focus_row.get('10日勝率%'),1)}% / {format_number(focus_row.get('20日勝率%'),1)}%", ""),
                ("5日 / 10日 / 20日平均報酬", f"{format_number(focus_row.get('5日平均報酬%'),1)}% / {format_number(focus_row.get('10日平均報酬%'),1)}% / {format_number(focus_row.get('20日平均報酬%'),1)}%", ""),
                ("最近買點", f"{_safe_str(focus_row.get('最近買點日期'))} / {_safe_str(focus_row.get('最近買點分級'))}", ""),
                ...
                ("買點回測摘要", _safe_str(focus_row.get("買點回測摘要")), ""),
    ...
    backtest_rank = rec_df.sort_values(["買點回測分數", "20日勝率%", "20日平均報酬%", "推薦總分"], ascending=[False, False, False, False]).reset_index(drop=True)
    tabs = st.tabs(["完整推薦表", "買點回測榜", "類股強度榜", "同類股領先榜", "自動因子榜", "飆股補抓", "操作說明"])

    with tabs[0]:
        full_default_cols = [
            "股票代號", "股票名稱", "市場別", "類別", "推薦模式", "推薦等級", "推薦總分",
            "市場環境分數", "型態名稱", "型態突破分數", "爆發等級", "爆發力分數",
            "技術結構分數", "起漲前兆分數", "交易可行分數", "買點分級", "買點回測分數", "20日勝率%", "20日平均報酬%", "類股熱度分數",
    ...
    with tabs[1]:
        backtest_cols = [
            "股票代號", "股票名稱", "市場別", "類別", "推薦模式", "推薦等級", "推薦總分",
            "買點分級", "買點回測分數", "回測樣本數", "最近買點日期", "最近買點分級",
            "5日勝率%", "10日勝率%", "20日勝率%",
            "5日平均報酬%", "10日平均報酬%", "20日平均報酬%",
            "起漲前兆分數", "交易可行分數", "型態名稱", "型態突破分數", "買點回測摘要"
        ]
        st.dataframe(_format_df(backtest_rank[[c for c in backtest_cols if c in backtest_rank.columns]].head(max(top_n, 30))), use_container_width=True, hide_index=True)
    ...
    with tabs[6]:
        render_pro_info_card(
            ...
            ("推薦策略", "新增 精準版 / 結合版；結合版會另外列出飆股補抓，不混入主名單。", ""),
                ...
                ("買點回測榜", "新增 S/A/B/C/D 買點分級、5/10/20日勝率與平均報酬，直接用歷史樣本驗證。", ""),
