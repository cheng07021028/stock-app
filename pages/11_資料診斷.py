...
st.markdown('---')
st.subheader('9. v42 全系統串聯驗證與欄位修復')
st.caption('檢查 0_大盤趨勢、7_股神推薦、8_股神推薦紀錄、10_推薦清單、首頁 / 儀表板之間的 JSON 串聯。此區塊只讀本機檔案，不重新抓網路資料；v42 可補齊舊推薦紀錄缺少的大盤欄位。')
...
try:
    ...
    with cc5:
        _render_metric_card('總項目', str(_v41_summary.get('總項目', 0)), 'v42 串聯檢查項目數', 'info')

    with st.expander('v42 橋接檔檢查：market_snapshot / macro bridge', expanded=True):
        st.dataframe(pd.DataFrame(_v41_report.get('bridge_rows', [])), use_container_width=True, hide_index=True)

    with st.expander('v42 大盤快照欄位檢查：0 大盤趨勢 -> 7 股神推薦', expanded=True):
    ...
    st.markdown('##### v42 舊推薦資料大盤欄位補齊')
    st.caption('用途：修正畫面上「7 -> 8 / 10」出現缺欄位造成異常數增加。此功能只補缺少或空白欄位，不刪除任何推薦紀錄。')
    if st.button('v42 一鍵補齊舊推薦資料的大盤欄位', use_container_width=True, type='primary'):
    ...
    st.error(f'v42 全系統串聯驗證載入失敗：{_v41_e}')
    st.code(traceback.format_exc())
