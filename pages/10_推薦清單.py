...
def _backfill_v10_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    v10_cols = [
        "股神決策模式", "股神進場建議", "進場時機", "進場時機分數", "建議動作", "等待條件", "操作區間", "追高風險等級", "是否建議追價", "推薦分層", "建議部位%", "建議倉位%", "建議投入等級", "分批策略", "最大風險%", "單檔風險等級", "族群集中警示", "組合配置建議", "風險報酬比", "追價風險分",
...
def _ensure_record_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    num_cols = [
        "推薦總分", "族群資金流分數", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "技術結構分數", "起漲前兆分數", "交易可行分數", "類股熱度分數", "強勢族群等級", "族群資金流分數", "族群輪動狀態", "同族群強勢比例", "同族群推薦密度", "同族群平均量能分", "族群策略建議", "族群資金流說明", 
    ...
    for c in ["推薦日期", "推薦時間", "建立時間", "更新時間", "最新更新時間", "目前狀態", "模式績效標籤", "命中結果", "績效評語", "追蹤更新時間", "備註"]:
...
def main():
    ...
    show_cols = [
        "資料來源", "推薦日期", "推薦時間", "股票代號", "股票名稱", "推薦模式", "推薦型態", "機會型態", "推薦等級", "推薦總分", "機會股分數", "低檔位置分數", "拉回承接分數", "支撐回測分數", "止跌轉強分數", "股神決策模式", "股神進場建議", "進場時機", "進場時機分數", "建議動作", "等待條件", "操作區間", "追高風險等級", "是否建議追價", "推薦分層", "建議部位%", "建議倉位%", "建議投入等級", "分批策略", "最大風險%", "單檔風險等級", "族群集中警示", "組合配置建議", "風險報酬比", "追價風險分", "飆股起漲分數", "起漲等級", "起漲摘要",
