...
def _phase80_render_actionable_panel(rec_df: pd.DataFrame) -> None:
    ...
    try:
        _h42_focus = build_focus_decision_table(decision_source, strong_top=2, bargain_top=2) if callable(build_focus_decision_table) else pd.DataFrame()
...
def _build_excel_bytes(
    ...
    try:
        focus_decision_df = build_focus_decision_table(candidate_source, strong_top=2, bargain_top=2) if callable(build_focus_decision_table) else pd.DataFrame()
