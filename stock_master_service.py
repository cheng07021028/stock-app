...
def _read_json_from_github(path: str) -> tuple[Any, str]:
    """
    優先讀 GitHub；若 GitHub token 未設定或讀取失敗，回退讀本機檔案。
    這可避免 Streamlit Cloud / 本機環境因 secrets 或 API 暫時失敗而整個主檔變空。
    """
    cfg = _stock_master_config()
    token = cfg["token"]

    def _read_local_json(local_path: str) -> tuple[Any, str]:
        candidates = [
            local_path,
            local_path.replace("/", "_"),
            "stock_master_cache.json" if "stock_master" in local_path else local_path,
            "stock_category_overrides.json" if "override" in local_path else local_path,
            f"data/{local_path}",
        ]
        seen = set()
        for p in candidates:
            ...
            try:
                ...
                fp = Path(p)
                if fp.exists():
                    return json.loads(fp.read_text(encoding="utf-8-sig")), f"local:{p}"
    ...
    try:
        ...
        if resp.status_code == 404:
            local_payload, local_msg = _read_local_json(path)
            return local_payload, local_msg if local_payload is not None else ""
        if resp.status_code != 200:
            local_payload, local_msg = _read_local_json(path)
            if local_payload is not None:
                return local_payload, f"GitHub 讀取失敗，已改用 {local_msg}"
        ...
        return json.loads(decoded), "github"
    except Exception as e:
        local_payload, local_msg = _read_local_json(path)
        if local_payload is not None:
            return local_payload, f"GitHub 例外，已改用 {local_msg}：{e}"
