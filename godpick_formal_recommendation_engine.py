...
def _exclusion_reasons(row: pd.Series) -> list[str]:
    ...
    候選強度高不代表可買；只要買點、風控、流動性或風險報酬失衡，
    就必須和正式推薦隔離。雷達回放仍可由後續函式保留，但不得升格。
    """
    reasons: list[str] = []
    ...
    if _contains_any(liquidity_blob, ["低流動性排除", "冷門禁追", "極低量"]):
