...
VERSION='v191_h87_autofresh_progress_settings_20260921'
...
DEFAULT_SETTINGS={
 ...
 'ttl_minutes':{'stock_master':10080,'macro_full':120,'super_ai_context':240,'watchlist_runtime':720,'performance_feedback':360,'h82_learning':360,'news':90},
...
def save_settings(settings):
 ...
 try:
  ...
  if not ok:return False,'H87設定未取得永久權威確認。'+msg,p
  v,_=load_settings()
  if int(v.get('update_seq',0) or 0)<p['update_seq']:return False,'H87設定寫入後回讀版本未更新。',p
  return True,f"H87自動新鮮度設定已永久保存（seq={p['update_seq']}）。{msg}",p
 except Exception as e:return False,f'H87設定保存失敗：{type(e).__name__}: {e}',p
