...
PAGE_TITLE = "股神管理中心｜v42 欄位管理按套用才運算版"
...
def _render_column_manager(table_key: str, table_label: str, schema_df: pd.DataFrame, default_cols: List[str]) -> List[str]:
    ...
    k_prefix = f"v42_colmgr_{table_key}"
    ...
    with st.expander(f"🧩 {table_label} 欄位管理 / v42 按套用才運算", expanded=False):
        ...
        with st.form(key=f"{k_prefix}_form", clear_on_submit=False):
            ...
            order_text = st.text_area(
                ...
                key=text_key,
                height=320,
                help="v42：在這裡輸入不會立即運算，按下方套用按鈕才會重排表格。",
            )
        ...
        if preview_btn or reset_btn or nonempty_btn:
            st.success("已套用到本次畫面；若要永久保存，請再按『套用並永久記錄』。")
            st.rerun()
...
def main() -> None:
    ...
    if render_pro_hero:
        try:
            render_pro_hero("股神管理中心", "v42｜欄位管理按套用才運算版：輸入順序不即時重算，按套用才永久保存")
        except Exception:
            st.title(PAGE_TITLE)
    else:
        st.title(PAGE_TITLE)
    st.caption("本頁整合 v18 投資組合、v19 每日追蹤、v20 推薦品質儀表板；不修改推薦邏輯、不寫入 JSON、不影響掃描速度。")
    st.caption("v42 修正：欄位管理改成表單模式，輸入欄位順序時不重算；只有按套用才重排表格與永久保存。")
