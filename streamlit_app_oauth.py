...
from google_drive_backup_oauth import (
    backup_github_repo_to_google_drive,
    disconnect_google_drive,
    get_google_auth_url,
    handle_google_oauth_callback,
    is_google_connected,
    list_recent_drive_backups,
)
from utils import (
    format_number,
...
def _render_backup_section():
    render_pro_section("GitHub 專案一鍵備份到 Google Drive（OAuth 版）")

    try:
        if handle_google_oauth_callback():
            st.success("Google Drive 授權成功")
    except Exception as e:
        st.error(f"Google Drive 授權失敗：{e}")
    ...
    st.caption("這版改用你的 Google 帳號授權，備份檔會上傳到你自己的 Google Drive。")
    ...
    with i3:
        st.caption(f"Google 狀態：{'已授權' if is_google_connected() else '尚未授權'}")

    if not is_google_connected():
        auth_url = get_google_auth_url()
        st.link_button("先用 Google 帳號登入並授權", auth_url, use_container_width=True)
        st.info("請先完成 Google 授權，再回到本頁按備份。")
        return

    b0, b1, b2 = st.columns([1, 1.5, 1])
    with b0:
        if st.button("中斷 Google 授權", use_container_width=True, key=_k("disconnect_btn")):
            disconnect_google_drive()
            st.success("已中斷 Google Drive 授權")
            st.rerun()
    with b1:
    ...
    if last_result:
        ...
        with r1:
            render_pro_info_card("最新備份時間", [("時間", _safe_str(last_result.get("backup_time_tw")), "")])
        with r2:
            render_pro_info_card("最新備份大小", [("bytes", f"{int(last_result.get('size_bytes', 0)):,}", "")])
        with r3:
            render_pro_info_card("最新Drive檔案", [("檔名", _safe_str(last_result.get("file_name")), "")])
    ...
    try:
        backup_rows = list_recent_drive_backups(root_folder_name=folder_name, limit=10)
    ...
    render_pro_info_card(
        "Google OAuth 設定提醒",
        [
            ("必要欄位", "GDRIVE_OAUTH_CLIENT_ID / GDRIVE_OAUTH_CLIENT_SECRET / GDRIVE_OAUTH_REDIRECT_URI", ""),
            ("登入方式", "用你的 Google 帳號授權，不再使用 service account quota", ""),
            ("資料夾ID", "可不填；不填時會自動建立 stock-app-backups", ""),
        ],
    )
...
def _render_home_page():
    ...
    with q1:
        render_pro_info_card("行情查詢", [("用途", "看單股即時資訊、訊號燈號、支撐壓力。", "")], chips=["pages/2_行情查詢.py"])
    with q2:
        render_pro_info_card("歷史K線分析", [("用途", "看 K 線、MA、KD、MACD、事件與策略。", "")], chips=["pages/3_歷史K線分析.py"])
    with q3:
        render_pro_info_card("自選股中心", [("用途", "新增 / 刪除 / 批次管理自選股。", "")], chips=["pages/4_自選股中心.py"])
    with q4:
        render_pro_info_card("多股比較 / 排行榜", [("用途", "做多檔比較、群組排行、找強弱股。", "")], chips=["pages/5_排行榜.py", "pages/6_多股比較.py"])
