...
try:
    from godpick_column_schema import (
        ...
        dedupe_keep_order as _schema_dedupe_keep_order,
        is_blank as _schema_is_blank,
        normalize_godpick_dataframe,
        standardize_records_for_storage,
    )
except Exception:
    UNIFIED_RECOMMEND_DISPLAY_COLUMNS = []
    _schema_dedupe_keep_order = None
    _schema_is_blank = None
    normalize_godpick_dataframe = None
    standardize_records_for_storage = None
...
# v29：推薦紀錄也採用與 7 / 8 / 10 / 12 共用欄位順序，避免寫入時把新欄位截掉。
if UNIFIED_RECOMMEND_DISPLAY_COLUMNS:
    if _schema_dedupe_keep_order is not None:
        GODPICK_RECORD_COLUMNS = _schema_dedupe_keep_order(list(GODPICK_RECORD_COLUMNS) + list(UNIFIED_RECOMMEND_DISPLAY_COLUMNS))
    else:
        GODPICK_RECORD_COLUMNS = list(dict.fromkeys(list(GODPICK_RECORD_COLUMNS) + list(UNIFIED_RECOMMEND_DISPLAY_COLUMNS)))
...
def ensure_record_columns(df: pd.DataFrame) -> pd.DataFrame:
    """v29：以共用欄位 schema 標準化，不再截掉 7/8/10/12 新欄位。"""
    ...
    try:
        if normalize_godpick_dataframe is not None:
            x = normalize_godpick_dataframe(x, add_missing=True, clean_none=True)
    except Exception:
        pass
    for c in GODPICK_RECORD_COLUMNS:
        if c not in x.columns:
            x[c] = ""
    # 只將真正數值欄轉成數值；文字欄一律保留 object，避免塞入「拉回可布局」等字串爆 dtype。
    num_cols = [
        "推薦總分", "推薦分數", "股神決策分數", "技術結構分數", "起漲前兆分數", "交易可行分數", "類股熱度分數",
        "同類股領先幅度", "推薦價格", "推薦日價格", "最新價", "推薦日價格", "停損價", "停損參考", "賣出目標1", "賣出目標2",
        "實際買進價", "實際賣出價", "實際報酬%", "損益金額", "損益幅%", "損益%", "持有天數",
        "建議倉位%", "動態建議倉位%", "第一筆進場%", "推薦後1日%", "推薦後3日%", "推薦後5日%", "推薦後10日%", "推薦後20日%",
    ]
    for c in num_cols:
        if c in x.columns:
            x[c] = pd.to_numeric(x[c], errors="coerce")
    bool_cols = ["是否領先同類股", "是否已實際買進", "是否已買進", "是否達停損", "是否達目標1", "是否達目標2", "是否達標_回測", "是否停損_回測"]
    for c in bool_cols:
        if c in x.columns:
            x[c] = x[c].fillna(False).map(_safe_bool)
    if "股票代號" in x.columns:
        x["股票代號"] = x["股票代號"].map(_normalize_code)
    # 避免 JSON 出現 NaN / None / dict/list 原生型別
    for c in x.columns:
        if c not in num_cols and c not in bool_cols:
            x[c] = x[c].map(_json_safe_value).astype("object")
    ordered = [c for c in GODPICK_RECORD_COLUMNS if c in x.columns]
    extras = [c for c in x.columns if c not in ordered and not str(c).startswith("_")]
    return x[ordered + extras].copy()
...
def build_record_from_row(row: pd.Series, mode_name: str = "股神推薦") -> dict[str, Any]:
    ...
    # v29：先把原始推薦列套用共用欄位標準，保留 7_股神推薦.py 已產生的完整資訊。
    schema_row: dict[str, Any] = {}
    try:
        if normalize_godpick_dataframe is not None:
            norm = normalize_godpick_dataframe(pd.DataFrame([row.to_dict() if hasattr(row, "to_dict") else dict(row)]), add_missing=True, clean_none=True)
            if not norm.empty:
                schema_row = norm.iloc[0].to_dict()
    except Exception:
        schema_row = {}
    ...
    # 不用空白覆蓋已計算值；有值的新欄位全部帶入紀錄。
    for k, v in schema_row.items():
        try:
            blank = _schema_is_blank(v) if _schema_is_blank is not None else (v is None or str(v).strip().lower() in {"", "none", "nan", "null"})
        except Exception:
            blank = False
        if not blank and k not in {"record_id"}:
            rec[k] = v
    # 主鍵與時間以紀錄服務產生者為準。
    rec.update({
        "record_id": rec_id,
    ...
    return ensure_record_columns(pd.DataFrame([rec])).iloc[0].to_dict()
...
def _write_records_to_github(df: pd.DataFrame) -> tuple[bool, str]:
    ...
    try:
        if standardize_records_for_storage is not None:
            work = standardize_records_for_storage(work, keep_extras=True)
    except Exception:
        pass
    content_text = json.dumps(work.to_dict(orient="records"), ensure_ascii=False, indent=2, allow_nan=False)
