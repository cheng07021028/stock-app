...
def _slice_by_focus(df: pd.DataFrame, event_df: pd.DataFrame, focus_event_idx: int, focus_window: str) -> pd.DataFrame:
    """
    v129：區間選擇以資料層裁切為主，避免只改 Plotly 前端 range 造成空白或顯示異常。
    重要：1M / 3M / 6M / 1Y / 全部 必須優先於事件焦點。
    只有未來若新增「事件模式」時，才讓事件焦點接管圖表。
    """
    if df is None or df.empty:
        return pd.DataFrame()

    work = df.copy().sort_values("日期").reset_index(drop=True)
    fw = _safe_str(focus_window).upper()
    tail_map = {"1M": 22, "3M": 66, "6M": 132, "1Y": 264, "30": 30, "60": 60, "120": 120, "240": 240}

    if fw in tail_map:
        n = min(len(work), int(tail_map[fw]))
        out = work.tail(n).reset_index(drop=True)
        if not out.empty:
            return out
    ...
    # 保留事件焦點能力，但不讓它覆蓋 1M/3M/6M/1Y/全部。
    if focus_event_idx is not None and focus_event_idx >= 0 and event_df is not None and not event_df.empty and focus_event_idx < len(event_df):
        ...
        if pd.notna(event_date):
            around = 30
            mask = (work["日期"] >= event_date - pd.Timedelta(days=around)) & (work["日期"] <= event_date + pd.Timedelta(days=around))
            focus_df = work.loc[mask].copy()
...
def _build_candlestick_chart(df: pd.DataFrame, stock_label: str, show_ma: bool, show_pivots: bool, peak_idx: tuple[int, ...], trough_idx: tuple[int, ...]) -> go.Figure:
    ...
    fig.update_xaxes(
        ...
        nticks=min(12, max(5, len(work) // 8)),
        row=1,
        col=1,
    )
    fig.update_xaxes(
...
def main():
    ...
    with st.expander("📈 圖表區間與顯示設定｜v128 保留套用並永久記錄", expanded=True):
        ...
        if apply_chart_settings:
            _old_window = _safe_str(st.session_state.get(_k("focus_window"), "6M"))
            _new_window = _safe_str(st.session_state.get(_k("focus_window_draft"), "6M"))
            _new_settings = {
                "event_filter": st.session_state.get(_k("event_filter_draft"), "全部"),
                "focus_window": _new_window,
            ...
            # v129：切換 1M/3M/6M/1Y/全部 時，清掉舊事件焦點，避免事件焦點覆蓋區間造成圖表顯示不正常。
            if _new_window != _old_window:
                st.session_state[_k("focus_event_idx")] = -1
        ...
        st.caption(f"目前套用：區間 {st.session_state.get(_k('focus_window'), '6M')}｜事件 {st.session_state.get(_k('event_filter'), '全部')}｜均線 {'開' if st.session_state.get(_k('show_ma'), True) else '關'}｜起漲起跌 {'開' if st.session_state.get(_k('show_pivots'), True) else '關'}｜v129 區間以資料層裁切並永久保存")
    ...
    if focus_df.empty:
        focus_df = df.copy()
    focus_df = focus_df.sort_values("日期").reset_index(drop=True)
