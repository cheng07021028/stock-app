# -*- coding: utf-8 -*-
"""Apply V191-H32 return forecast integration.

Changes:
1. Page07 enriches both recommendation rows and candidate-diagnosis rows with the
   probabilistic return forecast before persistence/export/automatic recording.
2. T+1 scheduler passes the already elected authority_rows into the truth service
   instead of letting it fall back to an empty local godpick_records.json.
3. T+1 truth stores H32 forecasts plus realized 5/10/20-session returns so future
   interval coverage / direction hit-rate / MAE can be measured out of sample.
4. H31 Excel guide surfaces H32 next-session/swing forecasts without changing
   Formal/A- authority.
"""
...
def main() -> None:
    ...
    patch_truth()
    patch_guide()
    print("PASS V191-H32 return forecast integration applied")
