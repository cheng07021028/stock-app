...
CANDIDATE_DIAGNOSIS_COLUMNS = [
    ...
    "盤前強勢前兆分", "強勢前兆分", "強勢前兆判定", "強勢前兆進場條件", "強勢前兆風控", "前置保留類型", "前置保留原因", "前置軟篩選狀態", "前置軟篩選階段", "前置軟篩選原因", "前置軟篩選數", "AI發現母體版本",
    "K線最後交易日", "K線落後交易日", "K線資料新鮮度", "本輪市場最新交易日", "K線日期驗證基準",
    # V191-H20：候選快照必須保留逐股官方日期/來源證據。舊版裁掉這些欄位後，
    # reboot / cold-cache 重建會再次把每檔股票視為「官方日期未驗證」。
    "官方資料日期", "官方因子資料日期", "官方因子落後交易日", "官方因子新鮮度",
    "官方因子資料狀態", "官方資料完整度", "官方因子資料源", "因子主要來源",
    "因子來源可信度", "每日因子來源可信度", "來源可信度狀態", "來源可信度說明",
    "法人資料日期", "營收資料日期", "估值資料日期", "FinMind資料日期",
    ...
    "技術結構分數", "起漲前兆分數", "交易可行分數", "類股熱度分數", "族群樣本可信度", "族群樣本校正說明",
    "類股內排名", "類股前3強", "是否領先同類股", "同類股領先幅度",
    "自動因子總分", "EPS代理分數", "營收動能代理分數", "獲利代理分數",
    "爆發雷達分", "隔日爆發分", "飆股攻擊分", "主流領漲回補分", "漲停回放分",
    "強勢股漏選風險分", "AI市場狀態", "AI召回路徑", "AI主要召回路徑", "AI召回分",
    "AI Alpha品質分", "AI Timing時機分", "AI Risk風控分", "AI Continuation延續分", "AI資料信心分",
    "AI綜合決策分", "AI跨市場新強股分", "AI新證據分", "AI召回來源數", "AI召回保留旗標", "AI漏選風險分",
    "AI結構停損距離%", "AI戰術停損距離%", "AI戰術風報比", "AI趨勢延伸目標價", "AI領漲延續狀態", "AI過熱型態", "AI風險口徑", "AI推薦資格",
    "H51推薦等級", "H51市場地位", "H51交易許可", "H51專業參考分", "H51可執行分", "H51族群主線分", "H51個股領漲品質分", "H51Pivot起漲分", "H51路徑RR", "H51推薦理由", "H51版本",
    "推薦角色", "飆股雷達角色", "領漲回補角色", "回放校正角色",
...
ACTION_TABLE_COLUMNS = [
    ...
    "AI跨市場新強股分", "AI新證據分", "AI召回保留旗標", "AI漏選風險分", "AI戰術停損距離%", "AI戰術風報比", "AI過熱型態", "AI推薦資格",
    "隔日可參考分", "隔日優勢型態", "隔日風險標記", "隔日參考判定", "觸發距離%", "停損距離_隔日%",
    "進場可執行分", "進場可執行判定", "進場路徑", "距最近可執行買點%", "進場阻擋原因",
    "K線最後交易日", "K線落後交易日", "K線資料新鮮度", "本輪市場最新交易日", "K線日期驗證基準",
...
def build_candidate_diagnosis(df: pd.DataFrame | None, *, max_rows: int = 3000) -> pd.DataFrame:
    ...
    extra = [
        col for col in [
            ...
            "H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分", "H50族群起漲候選比例%",
            "H50族群延伸過熱比例%", "H50族群重複壓力分", "H50族群生命週期", "H50波段機會階段",
            "H50主流購買優先分", "H50主流購買狀態", "H50重複推薦扣分", "H50主流購買理由", "H50主流版本", "H50版本",
            "H50推薦優先分", "H50推薦等級", "H50推薦決策", "H50推薦許可", "H50推薦重複扣分", "H50推薦理由",
            "H47主流領先狀態", "H47交易候選狀態", "H47起漲優先分", "H47個股相對強度分",
            "H47族群內領先排名", "H47族群內領先百分位%", "H47主流領先理由", "H47版本",
...
def build_engine_diagnostic_table(df: pd.DataFrame | None) -> pd.DataFrame:
    ...
    cols = [
        ...
        "H50族群可買主流分", "H50族群新鮮度分", "H50族群回檔再攻分", "H50族群生命週期", "H50波段機會階段",
        "H50主流購買優先分", "H50主流購買狀態", "H50推薦優先分", "H50推薦決策", "H50推薦許可", "H50推薦理由", "H50主流版本", "H50推薦版本", "H50版本",
        "H47主流領先狀態", "H47交易候選狀態", "H47起漲優先分", "H47個股相對強度分", "H47族群內領先排名", "H47族群內領先百分位%", "H47主流領先理由", "H47版本",
